...
Процедура ОформитьВозврат()
	...
	// Выбор вида возврата (Нал/Карта)
	СписокВозврата = Новый СписокЗначений;
	СписокВозврата.Добавить(PredefinedValue("Перечисление.ВидыОплаты.Наличные"), "Наличными");
	СписокВозврата.Добавить(PredefinedValue("Перечисление.ВидыОплаты.Безналичные"), "На карту");
	
	Оп = Новый ОписаниеОповещения("ВыборВидаВозвратаЗавершение", ЭтотОбъект);
	СписокВозврата.ПоказатьВыборЭлемента(Оп, "Выберите способ возврата денег:");
...
Процедура ВыборВидаВозвратаЗавершение(ВыбранныйЭлемент, Параметры) Экспорт
	...
	Если ОформитьВозвратНаСервере(ВидОплаты) Тогда
...
&НаСервере
Функция ОформитьВозвратНаСервере(Знач ВидОплаты)
	...
	Попытка
		...
		НовыйВозврат.ФискальныйВозврат = Истина; // Возврат через кассу всегда фискальный
		
		// Если есть чек-основание
...
Асинх Процедура ОбработкаВыбораРежимаВозврата(ВыбранныйЭлемент, ДопПараметры) Экспорт
	...
	Если ВыбранныйЭлемент.Значение = "ПоЧеку" Тогда
		...
		ВыбранныйЧек = Ждать ОткрытьФорму("Документ.ЧекККМ.Форма.ФормаВыбораЧека", ПараметрыФормы);
		
		Если ТипЗнч(ВыбранныйЧек) = "ДокументСсылка.ЧекККМ" Тогда
