...
| Реализация | Расширение **PTM_API** (`Конфигурация_PTM_API/`), не основная конфигурация |
...
```
Клиент
  → [расширение PTM_API] HTTP Апи_Внешний
    → Апи_Транспорт (auth, JSON, ошибки, request-id)
      → Апи_Регистры / Апи_Справочники (use-cases)
        → объекты основной конфы (запросы, документы)
```

- Код API живёт **только в расширении** `Конфигурация_PTM_API/`.
- HTTP-модуль: тонкий handler → application → `HTTPСервисОтвет`.
