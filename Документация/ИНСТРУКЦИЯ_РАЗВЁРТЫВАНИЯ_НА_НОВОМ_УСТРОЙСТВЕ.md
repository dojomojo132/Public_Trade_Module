...
| `_ptm_analyze.py` | `CONFIG_ROOT` (путь к Конфигурация/Проверка) |
...
1. Агент вносит изменения в XML/BSL файлы в `Конфигурация/` **И** `Конфигурация/Проверка/`
2. `deploy-config.ps1 -Action Full` загружает из `Проверка/` в ИБ
3. При ошибках — цикл исправления + повторный деплой
4. Мониторинг runtime-ошибок через `monitor-errors.ps1`

**КРИТИЧНО:** Изменения ОБЯЗАТЕЛЬНО вносить в ОБЕ папки (`Конфигурация/` и `Конфигурация/Проверка/`). Деплой загружает ТОЛЬКО из `Проверка/`!
...
**Причина:** Файлы изменены только в `Конфигурация/`, но не в `Конфигурация/Проверка/`.

**Решение:** Всегда вносить изменения в ОБЕ папки. Деплой загружает ТОЛЬКО из `Проверка/`.

Подробности в `Документация/КРИТИЧЕСКИЕ_ОШИБКИ.md` → запись от 2026-02-16.
...
3. При редактировании BSL/XML — вносить изменения в ОБЕ папки: `Конфигурация/` и `Конфигурация/Проверка/`
