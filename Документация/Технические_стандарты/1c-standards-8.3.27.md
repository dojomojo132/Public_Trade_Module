...
// Десериализация
ВосстановленнаяСсылка = ЗначениеИзСтрокиВнутр(СтрокаСсылки);
ВосстановленноеЧисло = ЗначениеИзСтрокиВнутр("125");
```

### 14.3 Сохранение/чтение произвольных значений
...
|-------|--------|--------------------|
| `ЗначениеВСтрокуВнутр` / `ЗначениеИзСтрокиВнутр` | Внутренний 1С | Хранение в БД |
| `XMLString` / `XMLValue` | XML | Обмен данными |
| `ЗаписатьJSON` / `ПрочитатьJSON` | JSON | REST API, внешние системы |
