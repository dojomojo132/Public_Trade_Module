...
Функция ДатаСеанса() Экспорт
	...
	Если ИспользуетсяБСП() Тогда
		...
		Возврат МодульОбщегоНазначенияКлиент.ДатаСеанса();
		// Конец Вызов БСП
		
	КонецЕсли;

	Поправка = МенеджерОборудованияКлиент.ПодключаемоеОборудование().ПоправкаКВремениСеанса;
	ДатаСеанса = ТекущаяДата() + Поправка; // АПК:143 ТекущаяДата() для вычисления времени сеанса
	Возврат ДатаСеанса;
...
Функция СистемаЛогирования() Экспорт
	Возврат МенеджерОборудованияКлиент.ПодключаемоеОборудование().СистемаЛогирования;
КонецФункции

Функция ЕстьДанныеДляЗаписиОперацийОборудования() Экспорт
	
	СистемаЛогирования = СистемаЛогирования();
...
// Показать предупреждение что драйвер не сертифицирован.
//
// Параметры:
//  ОписаниеОшибки - Строка - текст сообщения
//
Процедура ПоказатьПредупреждениеДрайверНеСертифицирован(ОписаниеОшибки = Неопределено) Экспорт
	
	СсылкаНаСертОборудование = "https://v8.1c.ru/tekhnologii/standartnye-biblioteki/1s-biblioteka-podklyuchaemogo-oborudovaniya/sertifitsirovannye-modeli-oborudovaniya/"; 
	
	ТекстСообщения = ?(ПустаяСтрока(ОписаниеОшибки), "", ОписаниеОшибки + Символы.ПС);
	ТекстСообщения = ТекстСообщения + НСтр("ru = 'Используемый драйвер оборудования не сертифицирован или снят с поддержки.'") + Символы.ПС +  
			НСтр("ru = 'Пожалуйста, обратитесь к поставщику драйвера.'") + Символы.ПС +  
			НСтр("ru = 'Со списком актуальных, сертифицированных драйверов можно ознакомиться'") + " ";
	МассивСтрок = Новый Массив;
	МассивСтрок.Добавить(Новый ФорматированнаяСтрока(ТекстСообщения));
	МассивСтрок.Добавить(Новый ФорматированнаяСтрока(Новый ФорматированнаяСтрока(НСтр("ru = 'на портале 1С.'"),,,, СсылкаНаСертОборудование)));
...
Функция ДоступноИспользованиеРазделенныхДанных() Экспорт
	...
	Если ИспользуетсяБСП() Тогда
		...
		// Вызов БСП
		МодульОбщегоНазначенияКлиент = ОбщийМодуль("ОбщегоНазначенияКлиент");
		Возврат МодульОбщегоНазначенияКлиент.ДоступноИспользованиеРазделенныхДанных();
		// Конец Вызов БСП
...
Функция ИмяКомпьютераКлиента() Экспорт
...
// Возвращает Истина, если используется подсистема "ЛогированиеОборудования" и эта подсистема существует в конфигурации.
// Предназначена для реализации вызова необязательной подсистемы (условного вызова).
//
// Возвращаемое значение:
//  Булево.
//
Функция ИспользуетсяПодсистемаЛогирования() Экспорт
	
	Возврат ПодсистемаСуществует("ПоддержкаОборудования.ЛогированиеОборудования");
	
КонецФункции

// Возвращает Истина, если используется подсистема "НесертифицированноеОборудование" и эта подсистема существует в конфигурации.
// Предназначена для реализации вызова необязательной подсистемы (условного вызова).
//
// Возвращаемое значение:
//  Булево.
//
Функция ИспользуетсяНесертифицированноеОборудование() Экспорт
	
	Возврат ПодсистемаСуществует("ПоддержкаОборудования.ПодключаемоеОборудование.НесертифицированноеОборудование");
...
// Начинает замер времени если используется подсистема Оценка производительности.
// 
// Параметры:
//  КлючеваяОперация - Строка - имя ключевой операции
//  ФиксироватьСОшибкой - Булево - признак автоматической фиксации ошибки. 
...
//  УникальныйИдентификатор - уникальный идентификатор замера, который позволяет идентифицировать замер.
//
Функция НачатьЗамерВремениБПО(КлючеваяОперация, ФиксироватьСОшибкой = Ложь, АвтоЗавершение = Ложь) Экспорт
