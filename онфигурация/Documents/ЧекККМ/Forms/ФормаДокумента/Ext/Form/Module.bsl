...
&НаКлиенте
Асинх Процедура ОплатитьНаличными(Команда)
	Ждать ДобавитьОплату(ПредопределенноеЗначение("Перечисление.ВидыОплаты.Наличные"));
КонецПроцедуры

&НаКлиенте
Асинх Процедура ОплатитьКартой(Команда)
	Ждать ДобавитьОплату(ПредопределенноеЗначение("Перечисление.ВидыОплаты.Безналичные"));
КонецПроцедуры

&НаКлиенте
Асинх Функция ДобавитьОплату(ВидОплаты)
	...
	// Проверяем, нужно ли подтверждение для кассы типа "Банковский счет"
	КассаОплаты = Объект.Касса;
	...
	Если ВидКассы = ПредопределенноеЗначение("Перечисление.ВидыКасс.БанковскийСчет") Тогда
		...
		ТекстВопроса = "Оплата через банковский счет (касса """ + Строка(КассаОплаты) + """)." 
			+ Символы.ПС + "Сумма к оплате: " + Формат(ОстатокКДоплате, "ЧЦ=15; ЧДЦ=2")
		...
		Если Результат = КодВозвратаДиалога.Да Тогда
			ВыполнитьДобавлениеОплаты(ВидОплаты, ОстатокКДоплате);
		Иначе
			Сообщение = Новый СообщениеПользователю;
...
&НаКлиенте
Процедура ВыполнитьДобавлениеОплаты(ВидОплаты, СуммаОплаты)
	
	НоваяОплата = Объект.Оплата.Добавить();
	НоваяОплата.ВидОплаты = ВидОплаты;
	НоваяОплата.Сумма = СуммаОплаты;
...
Функция ПолучитьДанныеНоменклатурыНаСервере(Номенклатура, Склад, ДатаДок)
	...
	Запрос.УстановитьПараметр("Период", ?(ЗначениеЗаполнено(ДатаДок), ДатаДок, ТекущаяДатаСеанса()));
