&НаКлиенте
Перем ПоследнийПрочитанныйТекст; // Чтобы не обрабатывать одно и то же 100 раз в секунду
...
Процедура ПриОткрытии(Отказ)
	...
	//ПоследнийПрочитанныйТекст = "";
	//// Проверяем буфер 2 раза в секунду
	//ПодключитьОбработчикОжидания("СлушатьБуферОбмена", 1);
	
КонецПроцедуры

//&НаКлиенте
//Процедура СлушатьБуферОбмена()
//	
//	ТекстБуфера = "";
//	
//	// ТРЮК: Читаем буфер через HTML-объект Windows
//	// Это работает быстрее и надежнее, чем асинхронные методы платформы
//	Попытка
//		HTML = Новый COMОбъект("htmlfile");
//		ТекстБуфера = HTML.ParentWindow.ClipboardData.GetData("text");
//	Исключение
//		// Если буфер занят или недоступен
//		Возврат;
//	КонецПопытки;
//	
//	Если ТекстБуфера = Неопределено ИЛИ ПустаяСтрока(ТекстБуфера) Тогда
//		Возврат;
//	КонецЕсли;
//	
//	// ПРОВЕРКА 1: Это тот же текст, что мы уже читали?
//	Если ТекстБуфера = ПоследнийПрочитанныйТекст Тогда
//		Возврат;
//	КонецЕсли;
//	
//	// ПРОВЕРКА 2: Есть ли наш секретный префикс?
//	// (Чтобы не сканировать слово "Привет", если кассир его скопировал)
//	Маркер = "";  
//	ЧистыйШтрихкод = Сред(ТекстБуфера, СтрДлина(Маркер) + 1);
//	
//	// Запоминаем, чтобы не считать его снова в следующем цикле таймера
//	ПоследнийПрочитанныйТекст = ТекстБуфера;
//	
//	// ОБРАБАТЫВАЕМ
//	ОбработатьШтрихкод(ЧистыйШтрихкод);

//	
//	Маркер = "";
//	//Если Лев(ТекстБуфера, СтрДлина(Маркер)) = Маркер Тогда
//	//	
//	//	// Отрезаем префикс, получаем чистый штрихкод
//	//	
//	//	// Опционально: Очищаем буфер, чтобы кассир случайно не вставил штрихкод в документ
