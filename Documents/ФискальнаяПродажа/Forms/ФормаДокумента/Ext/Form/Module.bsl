...
Процедура ОбработатьШтрихкод(Штрихкод)
	...
	Если НайденныеТовары.Количество() = 0 Тогда
		...
		// Здесь можно предложить создать товар или назначить ШК (feature request)
		Возврат;
		
	ИначеЕсли НайденныеТовары.Количество() = 1 Тогда
		// SmartScan: Однозначное совпадение
		ДобавитьТоварВКорзину(НайденныеТовары[0]);
		
	Иначе
		// SmartScan: Коллизия (несколько товаров). Выбор из списка.
		ПоказатьВыборИзСписка(Новый ОписаниеОповещения("ПослеВыбораТовараИзСписка", ЭтотОбъект), НайденныеТовары, "Выберите товар:");
	КонецЕсли;
