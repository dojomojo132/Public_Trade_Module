...
**MCP Fallback:** если MCP-вызов вернул ошибку — НЕ останавливаться. Использовать XML-файлы из `Конфигурация/` как источник структуры. Пометить `⚠️ MCP недоступен` в ответе и `ERROR MCP unavailable` в Trace.
