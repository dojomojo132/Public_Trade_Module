...
**КРИТИЧНО:** Каждый запрос пользователя = полный цикл от реализации до запуска конфигуратора. НИКОГДА не останавливаться на этапе "Файлы созданы".
...
ФАЗА 2: ДЕПЛОЙ И ОТЛАДКА (АВТОМАТИЧЕСКИ!)
  ...
  ЭТАП 6: ОТЛАДКА      → Разобрать ошибки → Исправить → Повторить деплой
      ...
      ↓
ФАЗА 3: ЗАВЕРШЕНИЕ
  ЭТАП 7: КОНФИГУРАТОР → deploy-config.ps1 -Action Designer → Открыть конфигуратор
  ЭТАП 8: ДОКУМЕНТАЦИЯ → Обновить спецификацию → Добавить Record в History
```

> **Агент ОБЯЗАН** выполнить все 3 фазы в рамках одного запроса. Не останавливаться на "файлы созданы" или "готово к загрузке".
> Пользователь ожидает открытый конфигуратор с загруженной конфигурацией как результат.

### 3.1 Команды деплоя

```powershell
# Полный цикл: валидация XML → загрузка → синтакс-контроль → обновление БД → запуск
...
| `EXIT_CODE: -2` | Таймаут (процесс убит) | XML критически повреждён |
...
✓ ДЕПЛОЙ: deploy-config.ps1 -Action Full → успешно
✓ КОНФИГУРАТОР: deploy-config.ps1 -Action Designer → открыт
