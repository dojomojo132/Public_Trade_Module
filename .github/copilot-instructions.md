...
**1Cv8.dt включён в git** — коммитится вместе с кодом при явном FEAT-коммите.
...
**В git включается:** код (XML/BSL) + `1Cv8.dt` (состояние базы на момент успешного теста)
...
├── 1Cv8.dt                ← Образ информационной базы (в git, обновляется с каждым FEAT-коммитом)
└── .github/
...
✓ GIT COMMIT: только после ДИАЛОГА 2 + git add -A + _git_commit.py (включает 1Cv8.dt)
```
---
