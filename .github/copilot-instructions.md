...
```powershell
# Команды git-бэкапа (выполняются ПЕРЕД написанием кода):
cd D:\Git\Public_Trade_Module
git add -A
git commit -m "BACKUP: <дата> перед <краткое описание задачи>"
git push
```

**Правила:**
- Коммит ОБЯЗАТЕЛЬНО включает ВСЕ файлы: `Конфигурация/`, `1Cv8.dt`, `Документация/`
- Формат сообщения коммита: `BACKUP: YYYY-MM-DD перед <краткое описание задачи>`
- Push ОБЯЗАТЕЛЕН — локальный коммит недостаточен (защита от потери данных на диске)
- При откате: `git log --oneline -10` → найти нужный коммит → `git checkout <hash> -- Конфигурация/`
