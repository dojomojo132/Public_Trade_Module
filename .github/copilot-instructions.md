...
```
1. MCP → mcp_mcp_1c_torgov_get_metadata_structure → получить АКТУАЛЬНУЮ структуру из ИБ
2. Сравнить с файлом на диске (read_file .xml)
3. Если структура в ИБ ОТЛИЧАЕТСЯ от файла → файл УСТАРЕЛ:
   a) Выгрузить актуальную конфигурацию: deploy-config.ps1 -Action Dump
   b) ИЛИ обновить файл вручную по данным MCP
   c) НИКОГДА не деплоить устаревший файл — он затрёт изменения в ИБ!
4. Только после синхронизации — вносить свои изменения
```
...
❌ ЗАПРЕЩЕНО: деплоить файлы без проверки актуальности через MCP (файлы могут затереть изменения из конфигуратора!)
...
✅ ОБЯЗАТЕЛЬНО: ПЕРЕД изменением объекта — сверить файл на диске с MCP (структура из ИБ в приоритете!)
✅ ОБЯЗАТЕЛЬНО: если файл устарел — синхронизировать с ИБ ПЕРЕД внесением изменений
...
ФАЗА 1: РЕАЛИЗАЦИЯ
  ...
  ЭТАП 1а: СИНХРОНИЗАЦИЯ → MCP → Получить актуальную структуру объекта из ИБ
                          → Сравнить с файлом на диске
                          → Если файл устарел — обновить файл по данным MCP
...
✓ СИНХРОНИЗАЦИЯ: MCP → get_metadata_structure → сравнить с файлом → если файл устарел — обновить! (ЭТАП 1а)
