...
✅ ОБЯЗАТЕЛЬНО: ПЕРЕД изменениями — git-бэкап (конфигурация + .dt) с коммитом и пушем
...
ФАЗА 0: ЗАЩИТА ДАННЫХ (АВТОМАТИЧЕСКИ!)
  ЭТАП 0а: GIT-БЭКАП → git add + git commit + git push текущего состояния
                       Сохраняет ВСЕ файлы конфигурации (XML/BSL) + .dt в git
                       Формат коммита: "BACKUP: <дата> перед <описание задачи>"
...
1. **Git-бэкап** — коммит+пуш всех файлов конфигурации (XML/BSL) и .dt
...
3. После отката:
   a. `git checkout` для восстановления конфигурационных файлов (XML/BSL)
...
### 3.1 Git-бэкап (ЭТАП 0а — ПЕРЕД любыми изменениями)

**КРИТИЧНО:** Перед КАЖДЫМ изменением конфигурации — закоммитить и запушить текущее состояние.

**ПРОБЛЕМА: PowerShell ломает кириллицу в `git commit -m`!**
Никогда не делать `git commit -m "кириллица"` напрямую через PowerShell — буквы выпадают.

**Правильный воркфлоу (2 шага):**

Шаг 1: Написать сообщение в `scripts/_commit_msg.txt` через `create_file`:
```
# Формат BACKUP (перед изменениями):
BACKUP: 2026-02-23 перед добавлением документа Заказ

# Формат FEATURE (после подтверждения в ДИАЛОГЕ 1):
...
- Формат BACKUP: `BACKUP: YYYY-MM-DD перед <описание>`
- Формат FEAT: `FEAT: <заголовок>\n\n<подробный список что сделано>`

**Откат конфигурационных файлов через git:**
```powershell
# Посмотреть последние бэкап-коммиты:
git log --oneline --grep="BACKUP:" -10

# Откатить файлы конфигурации к конкретному коммиту:
git checkout <commit-hash> -- "Конфигурация/"

# ИЛИ откатить всё к последнему бэкап-коммиту:
git log --oneline --grep="BACKUP:" -1  # найти хэш
git checkout <hash> -- "Конфигурация/" "1Cv8.dt"
```
...
├── 1Cv8.dt                ← Бэкап информационной базы
└── .github/
...
```
✓ GIT-БЭКАП: git add + commit + push ПЕРЕД изменениями (ЭТАП 0а)
...
✓ ОТКАТ: Если целостность нарушена → откат git + .dt → запись в КРИТИЧЕСКИЕ_ОШИБКИ.md
