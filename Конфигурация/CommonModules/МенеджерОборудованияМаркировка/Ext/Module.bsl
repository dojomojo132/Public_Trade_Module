...
Функция КодПланируемыйСтатусМаркируемогоТовара(ПланируемыйСтатусМаркируемогоТовара) Экспорт
	...
	Результат = Перечисления.ПланируемыйСтатусМаркируемогоТовара.КодыПланируемыхСтатусовМаркируемогоТовара();
...
Функция КодОтветаОИСМОСтатусеТовара(ОтветОИСМОСтатусеТовара) Экспорт
	...
	Результат = Перечисления.ОтветОИСМОСтатусеТовара.КодыОтветаОИСМОСтатусеТовара();
...
Функция ОтветОИСМОСтатусеТовараПоКоду(ОтветОИСМОСтатусеТовара) Экспорт
	...
	Результат = Перечисления.ОтветОИСМОСтатусеТовара.ОтветыОИСМОСтатусеТовараПоКоду();
	Возврат Результат.Получить(ОтветОИСМОСтатусеТовара);
...
Функция ПараметрыЗаписиОперацииПроверкиКМ() Экспорт
	...
	Результат.Вставить("ПродажаРазрешена", Ложь);
	Результат.Вставить("ПричинаЗапрета", "");
...
// Возвращает параметры записи результата проверки ЕГАИС в журнал проверки кода маркировки
// 
// Возвращаемое значение:
//   Структура:
//     * ИдентификаторФискальнойОперации - Строка - guid идентификатор фискальной операции
//     * Выполнено - Булево - результат операции выполнен
//     * ШтрихкодыПродукции - Соответствие из КлючИЗначение:
//         ** Ключ - Строка - марка в base64
//         ** Значение - Структура :
//            *** Наименование - Строка - наименование продукции
//     * ИдентификаторЗапроса - Строка - Идентификатор запроса ЕГАИС
//     * ВходящееСообщение - ОпределяемыйТип.СообщениеЕГАИСвБПО - тип сообщения ЕГАИС
//     * ИсходящееСообщение - ОпределяемыйТип.СообщениеЕГАИСвБПО - тип сообщения ЕГАИС
//     * СообщениеПередано - Булево - флаг того что сообщение передано успешно
//     * ТекстОшибки - Строка
//     * ПодписьЧека - Строка
//
Функция ПараметрыЗаписиРезультатаПроверкиЕГАИС() Экспорт
	
	Возврат РегистрыСведений.ОперацииПроверкиКМ.ПараметрыЗаписиРезультатаПроверкиЕГАИС();
	
КонецФункции

// Выполняет запись в журнал операций проверки КМ
//
// Параметры:
...
Процедура ДобавитьРезультатыОтправкиЕГАИС(ПараметрыЗаписи) Экспорт
	...
	РегистрыСведений.ОперацииПроверкиКМ.ДобавитьРезультатыОтправкиЕГАИС(ПараметрыЗаписи);
...
Функция РезультатПроверкиСведенийОТоваре(РезультатПроверки) Экспорт
	...
	РезультатыПроверки = МенеджерОборудованияМаркировкаКлиентСервер.ТаблицаПроверкиСведенийОТоваре();
...
Функция РезультатПроверкиСведенийОТовареПФ(РезультатПроверки) Экспорт
	...
	РезультатыПроверки = МенеджерОборудованияМаркировкаКлиентСервер.ТаблицаПроверкиСведенийОТовареПФ();
	Возврат РезультатыПроверки.Получить(РезультатПроверки);
...
Процедура УстановитьСрокХраненияОперацийПроверкиКМ() Экспорт 
	...
	Если НЕ ЗначениеЗаполнено(Константы.СрокХраненияОперацийПроверкиКМ.Получить()) Тогда
		Константы.СрокХраненияОперацийПроверкиКМ.Установить(Перечисления.ПериодХраненияИсторииОпераций.ВсеВремя);
	КонецЕсли;
...
Процедура ЗаполнитьДанныеЗапросПроверкиКода(Позиция) Экспорт
	...
	Если Не ПустаяСтрока(ЗапросПроверкиКода.ИдентификаторЗапроса) И Не ПустаяСтрока(ЗапросПроверкиКода.ВременнаяМетка) Тогда
		...
		Если Не ПустаяСтрока(ЗапросПроверкиКода.ИдентификаторЭкземпляра) Тогда
			ЗначениеРеквизита = ЗначениеРеквизита + "&Inst=" + ЗапросПроверкиКода.ИдентификаторЭкземпляра;
		КонецЕсли;   
		Если Не ПустаяСтрока(ЗапросПроверкиКода.ВерсияБазы) Тогда
			ЗначениеРеквизита = ЗначениеРеквизита + "&Ver=" + ЗапросПроверкиКода.ВерсияБазы;
		КонецЕсли;   
		Если ПустаяСтрока(ОтраслевойРеквизит.ЗначениеРеквизита) Тогда // 1265
			ОтраслевойРеквизит.ЗначениеРеквизита = ЗначениеРеквизита;
		Иначе
			ОтраслевойРеквизит.ЗначениеРеквизита = ОтраслевойРеквизит.ЗначениеРеквизита + "&" + ЗначениеРеквизита;    
		КонецЕсли;                                
	КонецЕсли;
...
Процедура ЗаполнитьОтраслевойРеквизитВетеринарныеПрепараты(Позиция) Экспорт
	
	ТипыТоваров = ОборудованиеЧекопечатающиеУстройства.ОтраслевойРеквизитЗаполняетсяДляТиповТоваров();
	ОтраслевойРеквизит = Позиция.ОтраслевойРеквизит;
	Если ТипыТоваров.ВетеринарныеПрепараты Тогда        
		// Заполнение отраслевого реквизита для ветеринарных препаратов при передаче рецепта.
		ОтраслевойРеквизит.ИдентификаторФОИВ = "030"; // 1262 
		ОтраслевойРеквизит.ДатаДокументаОснования = Дата("20240527"); // 1263 - 27.05.2024
		ОтраслевойРеквизит.НомерДокументаОснования = "675"; // 1264         
		ЗначениеРеквизита = ?(Позиция.ВетеринарноеТребование, "tm=VPT", "tm=VPR");       
		Если Не ПустаяСтрока(Позиция.РецептНомер) Тогда     
			ЗначениеРеквизита = ЗначениеРеквизита  + "&dn=" + Позиция.РецептНомер;   
		КонецЕсли; 
		РецептДата = ?(Позиция.РецептДата <> Неопределено, Формат(Позиция.РецептДата, "ДФ=""ггММдд"""), "");
		Если Не ПустаяСтрока(РецептДата) Тогда
			ЗначениеРеквизита = ЗначениеРеквизита  + "&dd=" + РецептДата;   
		КонецЕсли; 
		ОтраслевойРеквизит.ЗначениеРеквизита = ЗначениеРеквизита // 1265
	КонецЕсли; 
...
Функция СтатусОбработкиЗапросаКМПоКоду(КодОбработкиЗапроса) Экспорт
	...
	СтатусыОбработки = Перечисления.СтатусОбработкиЗапросаКМ.СтатусыПоКоду();
