...
// Возвращает значение настройки "Название основной БД"
// Используется кассой для формирования URL запросов к основной БД.
//
// Возвращаемое значение:
//   Строка - Имя публикации основной БД (например "ptm")
//
Функция ПолучитьНазваниеОсновнойБД() Экспорт
	
	ИмяНастройки = Справочники.НастройкиПоУмолчанию.НазваниеОсновнойБД;
	Значение = ПолучитьЗначениеНастройки(ИмяНастройки);
...
// Возвращает значение настройки "Название кассовой БД"
// Используется основной БД для идентификации кассового узла.
//
// Возвращаемое значение:
//   Строка - Имя публикации кассовой БД (например "ptm-kassa")
//
Функция ПолучитьНазваниеКассовойБД() Экспорт
	
	ИмяНастройки = Справочники.НастройкиПоУмолчанию.НазваниеКассовойБД;
	Значение = ПолучитьЗначениеНастройки(ИмяНастройки);
...
Функция ПолучитьВсеНастройки() Экспорт
	...
	Настройки.Вставить("НазваниеОсновнойБД", ПолучитьНазваниеОсновнойБД());
	Настройки.Вставить("НазваниеКассовойБД", ПолучитьНазваниеКассовойБД());
...
Функция ПолучитьДефолтныеЗначения()
	...
	Элемент = Новый Структура("Ключ, Значение", 
		Справочники.НастройкиПоУмолчанию.НазваниеОсновнойБД, 
		"");
	Дефолты.Добавить(Элемент);
	
	// Название кассовой БД - по умолчанию пустая строка
	Элемент = Новый Структура("Ключ, Значение", 
		Справочники.НастройкиПоУмолчанию.НазваниеКассовойБД, 
		"");
	Дефолты.Добавить(Элемент);
