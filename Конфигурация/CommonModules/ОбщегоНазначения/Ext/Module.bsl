...
// Цена номенклатуры на дату из РС.ЦеныНоменклатуры.
// Универсально: тип цен передаётся аргументом (необязательный).
//
// Параметры:
//  АктуальнаяДата - Дата - граница ПолучитьПоследнее
//  Номенклатура - СправочникСсылка.Номенклатура
//  ТипЦен - СправочникСсылка.ТипыЦен - если не заполнен, используется «Закупочная»
//
// Возвращаемое значение:
//  Число - цена; 0 если номенклатура/тип цен пуст или записи нет
//
Функция ПолучитьЦену(АктуальнаяДата, Номенклатура, ТипЦен = Неопределено) Экспорт
	...
	ТипЦенСсылка = ТипЦен;
	Если Не ЗначениеЗаполнено(ТипЦенСсылка) Тогда
		ТипЦенСсылка = Справочники.ТипыЦен.Закупочная;
	КонецЕсли;
	Если Не ЗначениеЗаполнено(ТипЦенСсылка) Тогда
		ТипЦенСсылка = Справочники.ТипыЦен.НайтиПоНаименованию("Закупочная");
	КонецЕсли;
	Если Не ЗначениеЗаполнено(ТипЦенСсылка) Тогда
		Возврат 0;
	КонецЕсли;
...
// Розничная (основная) цена на дату.
// Берёт ОсновнойТипЦен из настроек, иначе предопределённый «Розничная»,
// и вызывает ПолучитьЦену.
//
Функция ПолучитьРозничнуюЦену(АктуальнаяДата, Номенклатура) Экспорт
	
	ТипЦенСсылка = НастройкиПоУмолчанию.ПолучитьОсновнойТипЦен();
	Если Не ЗначениеЗаполнено(ТипЦенСсылка) Тогда
		ТипЦенСсылка = Справочники.ТипыЦен.Розничная;
	КонецЕсли;
	Если Не ЗначениеЗаполнено(ТипЦенСсылка) Тогда
		ТипЦенСсылка = Справочники.ТипыЦен.НайтиПоНаименованию("Розничная");
	КонецЕсли;
	
	Возврат ПолучитьЦену(АктуальнаяДата, Номенклатура, ТипЦенСсылка);
