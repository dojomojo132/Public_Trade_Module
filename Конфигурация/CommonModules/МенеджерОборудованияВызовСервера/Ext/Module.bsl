...
Функция СоответствиеТиповОборудования() Экспорт
	...
	Возврат МенеджерОборудования.СоответствиеТиповОборудования();
...
// Функция возвращает из переменной сеанса рабочее место клиента.
// 
// Параметры:
//   ЕстьПодключаемоеОборудование - Булево - возвращается Истина если в базе есть подключаемое оборудование.
//   ПараметрыПоискаРабочегоМеста - Структура
// 
// Возвращаемое значение:
//  СправочникСсылка.РабочиеМеста.
//
Функция РабочееМестоКлиента(ЕстьПодключаемоеОборудование = Истина, ПараметрыПоискаРабочегоМеста = Неопределено) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	РабочееМесто = ПараметрыСеанса.РабочееМестоКлиента;
	Если Не ЗначениеЗаполнено(РабочееМесто) Тогда
		ОбновитьРабочееМестоКлиента(ПараметрыПоискаРабочегоМеста);
		РабочееМесто = ПараметрыСеанса.РабочееМестоКлиента;
	КонецЕсли;
...
//  MACАдрес - Строка, Массив Из Строка - MAC адрес или массив MAC адресов 
//
// Возвращаемое значение:
...
Функция СоздатьРабочееМестоКлиента(Параметры) Экспорт
	...
	Если Параметры.Свойство("MACАдреса") И Параметры.MACАдреса.Количество() > 0 Тогда
		Для Каждого MACАдрес Из Параметры.MACАдреса Цикл
			Строка = РабочееМесто.MACАдреса.Добавить();
			Строка.Адрес = MACАдрес;
		КонецЦикла;
	КонецЕсли;
	...
	УстановитьРабочееМестоКлиента(РабочееМесто.Ссылка);
...
Функция ТипыИспользуемогоОборудованияТекущегоРабочегоМеста()  Экспорт
	...
	Если ЗначениеЗаполнено(РабочееМесто) Тогда
		...
		|	Справочник.ПодключаемоеОборудование КАК ПодключаемоеОборудование
		|ГДЕ
		|	ПодключаемоеОборудование.ТипПодключения = ЗНАЧЕНИЕ(Перечисление.ТипыПодключенияОборудования.ОбщийДоступ)
...
Функция ПолучитьПараметрыXML(Параметры, ТипОборудования = Неопределено, ЗаписьПараметров = Истина)  Экспорт
	...
	Возврат МенеджерОборудования.ПолучитьПараметрыXML(Параметры, ТипОборудования, ЗаписьПараметров);
...
Функция ДрайверПоставляетсяВКонфигурации(ИдентификаторУстройства) Экспорт
	...
	Возврат МенеджерОборудования.ДрайверПоставляетсяВКонфигурации(ИдентификаторУстройства);
...
Функция АнализТекстаОшибкиОборудования(ИдентификаторУстройства, Знач ПараметрыПодключения, Знач ТекстОшибки) Экспорт
	...
	Возврат МенеджерОборудования.АнализТекстаОшибкиОборудования(ИдентификаторУстройства, ПараметрыПодключения, ТекстОшибки);
...
// Устарела: следует использовать СоответствиеТиповОборудования.
// Получить соответствие типов оборудования
//
// Возвращаемое значение:
//   Соответствие из КлючИЗначение -
...
Функция ДоступноИнкассацияВУправлениеФискальнымУстройством() Экспорт
	...
	Возврат МенеджерОборудования.ДоступноИнкассацияВУправлениеФискальнымУстройством();
...
Функция ПараметрыТипыОборудования() Экспорт
	...
	Возврат МенеджерОборудованияПовтИсп.ПараметрыТипыОборудования();
...
// Возвращает рабочее место клиента, найденное по переданному отбору
// 
// Параметры:
//  ПараметрыПоискаРабочегоМеста - см. МенеджерОборудованияКлиент.ПодключаемоеОборудование
//  СоздатьРабочееМесто - Булево - Создать рабочее место
// 
// Возвращаемое значение:
//  СправочникСсылка.РабочиеМеста, Произвольный - Найденное рабочее место клиента, иначе ПустаяСсылка
Функция ОбновитьРабочееМестоКлиента(ПараметрыПоискаРабочегоМеста = Неопределено, СоздатьРабочееМесто = Ложь) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	МенеджерОборудования.ЗаполнитьАктуальныеРеквизитыРабочегоМеста(ПараметрыПоискаРабочегоМеста);
	
	ИдентификаторКлиента = "";
	ИмяКомпьютера = "";
	MACАдреса = "";
	Если ПараметрыПоискаРабочегоМеста <> Неопределено Тогда
		ИдентификаторКлиента = ПараметрыПоискаРабочегоМеста.ИдентификаторКлиента;
		ИмяКомпьютера = ПараметрыПоискаРабочегоМеста.ИмяКомпьютера;
		MACАдреса = ПараметрыПоискаРабочегоМеста.MACАдреса;
	КонецЕсли;
	
	ТекущееРМ = Справочники.РабочиеМеста.ПустаяСсылка();
	СписокРМ = НайтиРабочиеМестаПоИдентификатору(ИдентификаторКлиента, ИмяКомпьютера, MACАдреса);
	Если СписокРМ.Количество() > 0 Тогда
		ТекущееРМ = СписокРМ[0];
		МенеджерОборудования.ОбновитьРеквизитыРабочегоМеста(ТекущееРМ, ПараметрыПоискаРабочегоМеста);
	Иначе
		Если СоздатьРабочееМесто Тогда
			ТекущееРМ = СоздатьРабочееМестоКлиента(ПараметрыПоискаРабочегоМеста);
		КонецЕсли;
	КонецЕсли;
...
// Обновляет реквизиты рабочего места
// 
// Параметры:
...
Процедура ОбновитьРеквизитыРабочегоМеста(РабочееМесто, АктуальныеРеквизитыРабочегоМеста) Экспорт
	...
	МенеджерОборудования.ОбновитьРеквизитыРабочегоМеста(РабочееМесто, АктуальныеРеквизитыРабочегоМеста);
...
Процедура ОбменССервером(ДанныеДляСервера, Знач ДанныеДляСервераПоЗначению) Экспорт
	
	Если ОбщегоНазначенияБПО.ИспользуетсяКассоваяСмена() Тогда
		Если ДанныеДляСервера.Свойство("СтатусыКассовыхСмен") Тогда
			СтатусыКассовыхСмен = ДанныеДляСервера.СтатусыКассовыхСмен;
			МодульКассовыеСмены = ОбщегоНазначенияБПО.ОбщийМодуль("КассовыеСмены");
			Для Каждого Статус Из СтатусыКассовыхСмен Цикл
				СтатусСмены = МодульКассовыеСмены.СтатусПоследнейСмены(Статус.Ключ);
				СтатусыКассовыхСмен.Вставить(Статус.Ключ, СтатусСмены);
			КонецЦикла;
		КонецЕсли;
		
	КонецЕсли;
	
	Если ДанныеДляСервераПоЗначению.Свойство("ЗаписиЛогирования") Тогда
		ЗаписиЛогирования = ДанныеДляСервераПоЗначению.ЗаписиЛогирования;
		ПараметрыМетода = Новый Массив(1);
		ПараметрыМетода[0] = ЗаписиЛогирования;
		ФоновыеЗадания.Выполнить(
			"ЛогированиеОперацийБПО.ЗаписатьЛогОперацииНаСервере",
			ПараметрыМетода);
	КонецЕсли;
...
// Записывает архив логов драйвера для указанного подключаемого оборудования.
//
// Параметры:
//  ПодключаемоеОборудование - СправочникСсылка.ПодключаемоеОборудование - 
//      оборудование, для которого записываются логи
...
Процедура ЗаписатьАрхивЛоговДрайвера(ПодключаемоеОборудование, Знач Данные) Экспорт
	...
	РегистрыСведений.АрхивЛоговДрайверовОборудования.
		ЗаписатьАрхивЛоговДрайвера(ПодключаемоеОборудование, Данные);
...
//
Процедура ОтправитьОтветЗаписиАрхиваЛоговДрайвера(Знач ДанныеЗапроса, Знач РезультатЗагрузки) Экспорт

	Справочники.ПодключаемоеОборудование.
