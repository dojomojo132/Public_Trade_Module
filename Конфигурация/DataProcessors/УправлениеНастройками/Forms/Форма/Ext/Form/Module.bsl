...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Управление видимостью полей касс разделения
	Элементы.КассаНаличных.Видимость = РазделениеФискальныхЧеков;
	Элементы.КассаБезналичных.Видимость = РазделениеФискальныхЧеков;
...
Процедура ЗагрузитьНастройки()
	...
	КассаБезналичных = НастройкиПоУмолчанию.ПолучитьКассуБезналичных();
	...
	ЕстьПустыеНастройки = ЕстьПустыеНастройки ИЛИ (КассаБезналичных = Неопределено);
...
Процедура СохранитьНастройкиНаСервере()
	...
	НастройкиПоУмолчанию.УстановитьЗначениеНастройки(Справочники.НастройкиПоУмолчанию.КассаБезналичных, КассаБезналичных);
...
Процедура ИнициализироватьЗначенияПоУмолчанию()
	...
	Если КассаБезналичных = Неопределено Тогда
		...
		Если НЕ Результат.Пустой() Тогда
			Выборка = Результат.Выбрать();
			Выборка.Следующий();
			КассаБезналичных = Выборка.Ссылка;
		КонецЕсли;
	КонецЕсли;
