...
Асинх Процедура Заполнить(Команда)
	...
	Результат = Ждать ВопросАсинх("Табличная часть будет очищена и заполнена текущими остатками. Продолжить?", РежимДиалогаВопрос.ДаНет);
	Если Результат = КодВозвратаДиалога.Да Тогда
		ЗаполнитьНаСервере();
	КонецЕсли;
...
Функция ПолучитьДанныеНоменклатурыНаСервере(Номенклатура, Склад)
	...
	// 1. Получаем закупочную цену из последнего проведённого ПриходТовара
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	ПриходТовараТовары.Цена КАК Цена
		|ИЗ
		|	Документ.ПриходТовара.Товары КАК ПриходТовараТовары
		|ГДЕ
		|	ПриходТовараТовары.Номенклатура = &Номенклатура
		|	И ПриходТовараТовары.Ссылка.Проведен = ИСТИНА
		|УПОРЯДОЧИТЬ ПО
		|	ПриходТовараТовары.Ссылка.Дата УБЫВ";
	...
	Иначе
		// 2. Если прихода не было — ищем в регистре ЦеныНоменклатуры с типом "Закупочная"
		Запрос = Новый Запрос;
		Запрос.Текст = 
			"ВЫБРАТЬ ПЕРВЫЕ 1
