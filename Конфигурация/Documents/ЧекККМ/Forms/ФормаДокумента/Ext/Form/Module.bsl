...
Процедура ДобавитьОплату(ВидОплаты)
	...
	Если ВидКассы = ПредопределенноеЗначение("Перечисление.ВидыКасс.БанковскийСчет") Тогда
		...
		ПараметрыОповещения = Новый Структура("ВидОплаты, ОстатокКДоплате", ВидОплаты, ОстатокКДоплате);
...
Процедура ПодтверждениеОплатыБанкомЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		ВыполнитьДобавлениеОплаты(ДополнительныеПараметры.ВидОплаты, ДополнительныеПараметры.ОстатокКДоплате);
	Иначе
		Сообщение = Новый СообщениеПользователю;
