...
Функция РазобратьШтриховойКодТовараСлужебная(Знач Штрихкод, Знач ШтрихкодВBase64 = Ложь, КодыGS1 = Неопределено) Экспорт;
	...
	Если ДлинаШтрихкода = 41 
		Или ДлинаШтрихкода = 52 Тогда // Код товара в формате Data Matrix маркировки блок табачной продукции.
...
Функция КодВФорматеDataMatrix40БезGS(ШтриховойКодТовара, ДанныеКодаТовара)
	...
	Если ДлинаШтрихкода = 52 И Сред(ШтриховойКодТовара, 42, 3) = "240" Тогда
		КодТНВЭД = Сред(ШтриховойКодТовара, 45, 8); // КодТНВЭД.
	Иначе
		КодТНВЭД = "";
	КонецЕсли;
	...
	Если ТолькоЦифрыВСтроке(GTIN) И МенеджерОборудованияКлиентСервер.РассчитатьКонтрольныйСимволGTIN14(GTIN) = Прав(GTIN, 1) Тогда
		...
		Если Не ПустаяСтрока(КодТНВЭД) Тогда
			ДанныеКодаТовара.ПредставлениеШтрихкодаБезКриптоХвоста = ДанныеКодаТовара.ПредставлениеШтрихкодаБезКриптоХвоста + "240" + КодТНВЭД;
		КонецЕсли;
		Возврат Истина;
