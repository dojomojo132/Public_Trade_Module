...
Процедура ПередЗаписью(Отказ)
	...
	// [Spec-First 1.2] Автозаполнение ПроцентНаценки из группы-родителя
	// Если элемент новый (или наценка не задана) и есть родительская группа —
	// берём наценку из группы, чтобы не задавать её вручную каждый раз
	Если НЕ ЭтоГруппа И ПроцентНаценки = 0 И ЗначениеЗаполнено(Родитель) Тогда
		ЗаполнитьНаценкуИзГруппы();
	КонецЕсли;
...
// Автоматически заполняет ПроцентНаценки из родительской группы номенклатуры.
// Ищет первую группу в иерархии родителей, у которой заполнена наценка.
// Если наценка найдена — устанавливает её в текущем объекте.
//
Процедура ЗаполнитьНаценкуИзГруппы()
	
	ТекущийРодитель = Родитель;
	
	Пока ЗначениеЗаполнено(ТекущийРодитель) Цикл
		
		НаценкаГруппы = ТекущийРодитель.ПроцентНаценки;
		
		Если НаценкаГруппы <> 0 Тогда
			ПроцентНаценки = НаценкаГруппы;
			Возврат;
		КонецЕсли;
