...
Процедура ОбработкаДанныхБуфера(ТекстИзБуфера)
    ...
    Для Инд = 1 По МассивСтрок.ВГраница() Цикл
        ...
        Если Колонки.Количество() > 4 Тогда
            ЭтоГруппаСтр = ВРег(СокрЛП(Колонки[4]));
        ...
        Если Колонки.Количество() > 5 Тогда
            НоваяСтрока.РодительКод = СокрЛП(Колонки[5]);
        КонецЕсли;
        
        // 7 — Родитель.Наименование
        Если Колонки.Количество() > 6 Тогда
            НоваяСтрока.РодительНаименование = СокрЛП(Колонки[6]);
        КонецЕсли;
...
Процедура СоздатьГруппу(Код, Название)
	...
	НомГруппа.Код = Формат(Число(Код), "ЧРГ=''; ЧГ=0");
	НомГруппа.Записать();
...
Процедура ЗаполнитьНоменклатуру(Код, Наименование, НаЭтикетке, ЕдИзмерения, Штрихкод, Родитель=Неопределено)
	...
	Ном.Код = Формат(Число(Код), "ЧРГ=''; ЧГ=0");          
	Ном.Артикул = Ном.Код;

	Ном.Наименование = СокрЛП(Наименование);
	//Ном.НазваниеНаЭтикетке = СокрЛП(НаЭтикетке);
	Ном.ЕдиницаИзмерения = ОпределитьЕдИзмерения(ЕдИзмерения);
	Ном.НалоговаяГруппа = Справочники.НалоговыеГруппы.А;  
...
Функция ПоискПоКоду(Код)
	...
	Попытка 
		...
		Ном = Справочники.Номенклатура.НайтиПоКоду(Формат(Число(Код),"ЧРГ=''; ЧГ=0"));
	...
	//Ном = Справочники.Номенклатура.НайтиПоКоду(КодПоиска);
	//
