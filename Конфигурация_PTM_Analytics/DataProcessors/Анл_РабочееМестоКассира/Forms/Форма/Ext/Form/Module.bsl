...
Процедура ДобавитьТоварВЧек(Знач Номенклатура, Знач АкцизнаяМарка, Знач Штрихкод = "")
	...
	// ФОП для отображения в строке ТЧ
	Если ЗначениеЗаполнено(Номенклатура.ФОП) Тогда
		СтрокаТЧ.ФОП = Строка(Номенклатура.ФОП);
	КонецЕсли;
...
Функция ОформитьЧекиСРазделениемПоФОПНаСервере(Знач ВидОплаты, Знач КассаНаличных)
	...
	Для Каждого СтрТЧ Из Объект.ТЧ Цикл
		...
		// Получаем ФОП из карточки номенклатуры
		ФОПНоменклатуры = СтрТЧ.Номенклатура.ФОП;
		
		Если Не ЗначениеЗаполнено(ФОПНоменклатуры) Тогда
			Сообщить("Не указан ФОП для номенклатуры """ + Строка(СтрТЧ.Номенклатура) + """! Укажите ФОП в карточке товара.");
			Возврат Результат;
		КонецЕсли;
