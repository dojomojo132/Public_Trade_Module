...
Процедура ДобавитьТоварВЧек(Знач Номенклатура, Знач АкцизнаяМарка, Знач Штрихкод = "")
	...
	Если ТребуетМарку И ФискальныйРежим Тогда
		...
		ОшибкаОстатка = ПроверитьФискальныйОстатокПриДобавлении(Номенклатура, 1);
		Если ЗначениеЗаполнено(ОшибкаОстатка) Тогда
			ВызватьИсключение(ОшибкаОстатка);
		КонецЕсли;
	...
	// Примечание: используем запрос вместо ПолучитьПоследнее(), т.к. регистр непериодический
	Если ЗначениеЗаполнено(Номенклатура.НалоговаяГруппа) Тогда
		Запрос = Новый Запрос(
			"ВЫБРАТЬ ПЕРВЫЕ 1 
			|	ФОП
			|ИЗ РегистрСведений.РаспределениеНалоговыхГрупп
			|ГДЕ НалоговаяГруппа = &НалоговаяГруппа"
		);
		Запрос.УстановитьПараметр("НалоговаяГруппа", Номенклатура.НалоговаяГруппа);
		ВыборкаРезультата = Запрос.Выполнить().Выбрать();
		
		Если ВыборкаРезультата.Следующий() Тогда
			Если ЗначениеЗаполнено(ВыборкаРезультата.ФОП) Тогда
				СтрокаТЧ.ФОП = Строка(ВыборкаРезультата.ФОП);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Функция ПроверитьФискальныйОстатокПриДобавлении(Номенклатура, ДобавляемоеКоличество)
	
	Если НЕ Номенклатура.ТребуетАкцизнуюМарку Тогда
		Возврат "";
	КонецЕсли;
	
	Попытка
		Если НЕ Фскл_ОстаткиСервер.КонтрольВключен() Тогда
			Возврат "";
		КонецЕсли;
	Исключение
		Возврат "";
	...
	ФОП = Фскл_ОстаткиСервер.ПолучитьФОППоНалоговойГруппе(Номенклатура.НалоговаяГруппа);
	КоличествоВЧеке = 0;
	
	Для Каждого СтрТЧ Из Объект.ТЧ Цикл
		Если СтрТЧ.Номенклатура = Номенклатура Тогда
			КоличествоВЧеке = КоличествоВЧеке + СтрТЧ.Количество;
		КонецЕсли;
	КонецЦикла;
	
	Проверка = Фскл_ОстаткиСервер.ПроверитьДостаточность(
		ФОП,
		Номенклатура,
		КоличествоВЧеке + ДобавляемоеКоличество,
		ТекущаяДатаСеанса());
	
	Если НЕ Проверка.Достаточно Тогда
		Возврат Проверка.ТекстОшибки;
	КонецЕсли;
...
&НаСервере
Функция СобратьТаблицуТоваровДляФискальнойПроверки()
	
	Таблица = Новый ТаблицаЗначений;
	Таблица.Колонки.Добавить("Номенклатура");
	Таблица.Колонки.Добавить("Количество");
	Таблица.Колонки.Добавить("НалоговаяГруппа");
	
	Для Каждого СтрТЧ Из Объект.ТЧ Цикл
		НоваяСтрока = Таблица.Добавить();
		НоваяСтрока.Номенклатура = СтрТЧ.Номенклатура;
		НоваяСтрока.Количество = СтрТЧ.Количество;
		НоваяСтрока.НалоговаяГруппа = СтрТЧ.Номенклатура.НалоговаяГруппа;
	КонецЦикла;
...
// Цена для РМК: РС.ЦеныНоменклатуры с отбором по типу цен
// (ОсновнойТипЦен из настроек, иначе предопределённый «Розничная»).
// См. ОбщегоНазначения.ПолучитьРозничнуюЦену.
//
&НаСервере
Функция ПолучитьЦенуНоменклатуры(Номенклатура)
	
	Возврат ОбщегоНазначения.ПолучитьРозничнуюЦену(ТекущаяДатаСеанса(), Номенклатура);
...
Функция ОформитьЧекиСРазделениемПоФОПНаСервере(Знач ВидОплаты, Знач КассаНаличных)
	...
	Попытка
		ОтказФиск = Ложь;
		Если НЕ Фскл_ОстаткиСервер.ПроверитьТоварыДляПродажи(
			СобратьТаблицуТоваровДляФискальнойПроверки(),
			ТекущаяДатаСеанса(),
			ОтказФиск) Тогда
			Возврат Результат;
		КонецЕсли;
	Исключение
		// Расширение PTM_Fiscal не подключено — контроль на фискализации
	КонецПопытки;
	...
	Для Каждого СтрТЧ Из Объект.ТЧ Цикл
		...
		Если ЗначениеЗаполнено(НалГруппа) Тогда
			...
			Запрос.УстановитьПараметр("НалоговаяГруппа", НалГруппа);
			ВыборкаРезультата = Запрос.Выполнить().Выбрать();
			
			Если ВыборкаРезультата.Следующий() Тогда
				ФОПНоменклатуры = ВыборкаРезультата.ФОП;
			КонецЕсли;
		КонецЕсли;
