...
Функция ГлавнаяСтраницаGET(Запрос)
...
Функция КорзинаGET(Запрос)
	...
	ЗапросДанных.Текст =
		"ВЫБРАТЬ
		|	Корзина.НомерСтроки КАК НомерСтроки,
	...
	Пока Выборка.Следующий() Цикл
		...
		Товар.Вставить("line", Выборка.НомерСтроки);
		...
		Если Выборка.НомерСтроки < 10000 Тогда
...
Функция КорзинаPOST(Запрос)
	...
	ЗапросМакс.Текст =
		"ВЫБРАТЬ
		|	ЕСТЬNULL(МАКСИМУМ(Корзина.НомерСтроки), 9999) КАК МаксНомер
		|ИЗ
		|	РегистрСведений.Анл_МобильнаяКорзина КАК Корзина
		|ГДЕ
		|	Корзина.ИдСессии = &ИдСессии
	...
	НаборЗаписей.Отбор.НомерСтроки.Установить(НовыйНомер);
	НоваяЗапись = НаборЗаписей.Добавить();
	НоваяЗапись.ИдСессии = ИдСессии;
	НоваяЗапись.НомерСтроки = НовыйНомер;
...
Функция КорзинаDELETE(Запрос)
	...
	Пока Выборка.Следующий() Цикл
		...
		НаборЗаписей.Отбор.НомерСтроки.Установить(Выборка.НомерСтроки);
		НаборЗаписей.Записать();
	КонецЦикла;
...
Функция СформироватьHTMLЗаголовок()
	...
	Стр = Стр + "    .scanner-area { padding: 16px; }" + Символы.ПС;
	Стр = Стр + "    #reader { width: 100%; }" + Символы.ПС;
...
Функция СформироватьHTMLСкрипт()
	...
	Стр = Стр + "    let html5QrcodeScanner = null;" + Символы.ПС;
	...
	Стр = Стр + "      document.getElementById('scannerArea').style.display = 'block';" + Символы.ПС;
	...
	Стр = Стр + "      initScanner();" + Символы.ПС;
	Стр = Стр + "      loadCart();" + Символы.ПС;
	Стр = Стр + "    }" + Символы.ПС;
	Стр = Стр + "    function initScanner() {" + Символы.ПС;
	Стр = Стр + "      try {" + Символы.ПС;
	Стр = Стр + "        html5QrcodeScanner = new Html5QrcodeScanner('reader', {" + Символы.ПС;
	Стр = Стр + "          fps: 10," + Символы.ПС;
	Стр = Стр + "          qrbox: { width: 250, height: 150 }," + Символы.ПС;
	Стр = Стр + "          formatsToSupport: [" + Символы.ПС;
	Стр = Стр + "            Html5QrcodeSupportedFormats.EAN_13," + Символы.ПС;
	Стр = Стр + "            Html5QrcodeSupportedFormats.EAN_8," + Символы.ПС;
	Стр = Стр + "            Html5QrcodeSupportedFormats.CODE_128," + Символы.ПС;
	Стр = Стр + "            Html5QrcodeSupportedFormats.CODE_39," + Символы.ПС;
	Стр = Стр + "            Html5QrcodeSupportedFormats.UPC_A," + Символы.ПС;
	Стр = Стр + "            Html5QrcodeSupportedFormats.QR_CODE" + Символы.ПС;
	Стр = Стр + "          ]" + Символы.ПС;
	Стр = Стр + "        });" + Символы.ПС;
	Стр = Стр + "        html5QrcodeScanner.render(onScanSuccess, onScanFailure);" + Символы.ПС;
	Стр = Стр + "      } catch(e) {" + Символы.ПС;
	Стр = Стр + "        document.getElementById('scannerArea').innerHTML = '<p style=""color:#f44336;padding:16px"">Камера недоступна. Используйте ручной ввод.</p>';" + Символы.ПС;
	Стр = Стр + "      }" + Символы.ПС;
	Стр = Стр + "    }" + Символы.ПС;
	Стр = Стр + "    function onScanSuccess(decodedText) { addByBarcodeValue(decodedText); }" + Символы.ПС;
	Стр = Стр + "    function onScanFailure(error) { }" + Символы.ПС;
