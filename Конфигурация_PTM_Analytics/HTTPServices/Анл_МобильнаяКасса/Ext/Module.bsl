...
Функция cartPOST(Запрос)
	...
	// Режим обновления количества
	Если ДанныеЗапроса.Свойство("line") И ДанныеЗапроса.Свойство("qty") Тогда
		НомерПоз = ДанныеЗапроса.line;
		НовоеКоличество = ДанныеЗапроса.qty;
		
		Если НовоеКоличество <= 0 Тогда
			Возврат СформироватьОтветОшибки(400, "Количество должно быть больше 0");
		КонецЕсли;
		
		НаборЗаписей = РегистрыСведений.Анл_МобильнаяКорзина.СоздатьНаборЗаписей();
		НаборЗаписей.Отбор.ИдСессии.Установить(ИдСессии);
		НаборЗаписей.Отбор.НомерПозиции.Установить(НомерПоз);
		НаборЗаписей.Прочитать();
		Для Каждого Запись Из НаборЗаписей Цикл
			Запись.Количество = НовоеКоличество;
			Запись.Сумма = Запись.Цена * НовоеКоличество;
		КонецЦикла;
	...
	Если НЕ ЗначениеЗаполнено(Штрихкод) Тогда
		Возврат СформироватьОтветОшибки(400, "Не указан barcode");
	КонецЕсли;
...
Функция cartDELETE(Запрос)
	...
	// Удаление конкретной позиции по номеру
	НомерПозиции = Запрос.ПараметрыЗапроса.Получить("line");
	Если ЗначениеЗаполнено(НомерПозиции) Тогда
		НаборЗаписей = РегистрыСведений.Анл_МобильнаяКорзина.СоздатьНаборЗаписей();
		НаборЗаписей.Отбор.ИдСессии.Установить(ИдСессии);
		НаборЗаписей.Отбор.НомерПозиции.Установить(Число(НомерПозиции));
		НаборЗаписей.Записать();
...
Функция searchGET(Запрос)
	...
	СтрокаПоиска = Запрос.ПараметрыЗапроса.Получить("q");
	Если НЕ ЗначениеЗаполнено(СтрокаПоиска) Тогда
		Возврат СформироватьОтветОшибки(400, "Не указан параметр q");
	КонецЕсли;
	...
	ЗапросДанных.УстановитьПараметр("Шаблон", "%" + СтрокаПоиска + "%");
	...
	Пока Выборка.Следующий() Цикл
		Цена = ОбщегоНазначения.ПолучитьРозничнуюЦену(ТекущаяДатаСеанса(), Выборка.Ссылка);
		...
		ЗапросШК.УстановитьПараметр("Ссылка", Выборка.Ссылка);
		РезультатШК = ЗапросШК.Выполнить();
		...
		ДанныеТовара.Вставить("barcode", Штрихкод);
	...
	Данные = Новый Структура;
	Данные.Вставить("success", Истина);
	Данные.Вставить("count", МассивТоваров.Количество());
...
Функция statusGET(Запрос)
	...
	// Проверяем маркерную запись НомерПозиции = 0 — признак подключённой РМК
	ЗапросМаркера = Новый Запрос;
	ЗапросМаркера.Текст =
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	Корзина.ИдСессии КАК ИдСессии
		|ИЗ
		...
		|	Корзина.ИдСессии = &ИдСессии
		|	И Корзина.НомерПозиции = 0";
	ЗапросМаркера.УстановитьПараметр("ИдСессии", ИдСессии);
	
	РМКАктивна = НЕ ЗапросМаркера.Выполнить().Пустой();
	
	Данные = Новый Структура;
	Данные.Вставить("success", Истина);
	Данные.Вставить("active", РМКАктивна);
...
Функция loginPOST(Запрос)
	...
	ПарольВшестнадцатеричном = ДанныеЗапроса.password;
	Если НЕ ЗначениеЗаполнено(ПарольВшестнадцатеричном) Тогда
		Возврат СформироватьОтветОшибки(400, "Не указан пароль");
	КонецЕсли;
	
	// Ищем пользователя по SHA-256 хешу пароля (хеш приходит с клиента)
	...
	ЗапросПользователя.Текст =
		...
		|ГДЕ
		|	ВЫРАЗИТЬ(Пользователи.ПарольМобильнойКассы КАК Строка(64)) = &Пароль
		|	И НЕ Пользователи.ПометкаУдаления";
	ЗапросПользователя.УстановитьПараметр("Пароль", НРег(СокрЛП(ПарольВшестнадцатеричном)));
	
	Попытка
		Результат = ЗапросПользователя.Выполнить();
	Исключение
		Возврат СформироватьОтветОшибки(500, "Ошибка сервера: " + ОписаниеОшибки());
	КонецПопытки;
	
	Если Результат.Пустой() Тогда
		Возврат СформироватьОтветОшибки(401, "Неверный пароль");
	КонецЕсли;
	...
	Данные = Новый Структура;
	Данные.Вставить("success", Истина);
	Данные.Вставить("user", СокрЛП(Выборка.Наименование));
	
	Возврат СформироватьОтветJSON(200, Данные);
	
КонецФункции

Функция rmkItemsGET(Запрос)
	...
	// Получаем только товары из РМК (НомерПозиции < 10000, НомерПозиции > 0)
	ЗапросДанных = Новый Запрос;
	ЗапросДанных.Текст =
		"ВЫБРАТЬ
		|	Корзина.НомерПозиции КАК НомерПозиции,
	...
	Пока Выборка.Следующий() Цикл
		...
		Товар.Вставить("source", "rmk");
		Товары.Добавить(Товар);
		СуммаЧека = СуммаЧека + Выборка.Сумма;
	КонецЦикла;
	
	Данные = Новый Структура;
	Данные.Вставить("success", Истина);
	Данные.Вставить("items", Товары);
	Данные.Вставить("count", Товары.Количество());
	Данные.Вставить("total", СуммаЧека);
	
	Возврат СформироватьОтветJSON(200, Данные);
	
КонецФункции

Функция sessionsGET(Запрос)
	
	// Получаем список всех активных сессий РМК (маркер НомерПозиции = 0)
	// и для каждой - сумму и кол-во товаров РМК
	ЗапросСессий = Новый Запрос;
	ЗапросСессий.Текст =
		"ВЫБРАТЬ
		|	МаркерыСессий.ИдСессии КАК ИдСессии,
		|	ЕСТЬNULL(СУММА(Корзина.Сумма), 0) КАК СуммаЧека,
		|	ЕСТЬNULL(КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Корзина.НомерПозиции), 0) КАК КоличествоТоваров
		|ИЗ
		|	РегистрСведений.Анл_МобильнаяКорзина КАК МаркерыСессий
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.Анл_МобильнаяКорзина КАК Корзина
		|		ПО МаркерыСессий.ИдСессии = Корзина.ИдСессии
		|			И Корзина.НомерПозиции > 0
		|			И Корзина.НомерПозиции < 10000
		|ГДЕ
		|	МаркерыСессий.НомерПозиции = 0
		|СГРУППИРОВАТЬ ПО
		|	МаркерыСессий.ИдСессии";
	
	Попытка
		Результат = ЗапросСессий.Выполнить();
	Исключение
		Возврат СформироватьОтветОшибки(500, "Ошибка запроса сессий: " + ОписаниеОшибки());
	КонецПопытки;
	Выборка = Результат.Выбрать();
	
	СессииМассив = Новый Массив;
	Пока Выборка.Следующий() Цикл
		Сессия = Новый Структура;
		Сессия.Вставить("id", Выборка.ИдСессии);
		Сессия.Вставить("total", Выборка.СуммаЧека);
		Сессия.Вставить("count", Выборка.КоличествоТоваров);
		СессииМассив.Добавить(Сессия);
	КонецЦикла;
	
	Данные = Новый Структура;
	Данные.Вставить("success", Истина);
	Данные.Вставить("sessions", СессииМассив);
