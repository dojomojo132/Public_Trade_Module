...
```
MCP → {config.mcp.onec.prefix}_list_metadata_objects
...
```
MCP → {config.mcp.onec.prefix}_get_metadata_structure
...
```
MCP → {config.mcp.onec.prefix}_get_form_structure
      metaType: "Catalog" / "Document" / ...
      name: "ИмяОбъекта"
...
```
MCP → {config.mcp.onec.prefix}_get_connected_objects
      metaType: "Catalog" / "Document" / ...
      name: "ИмяОбъекта"
```
...
```
MCP → {config.mcp.onec.prefix}_get_configuration_overview
```
