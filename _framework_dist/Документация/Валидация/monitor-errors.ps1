...
param(
    ...
    [int]$LastMinutes = 30,
    [string]$BasePath = ""  # Установите путь к ИБ (или передайте параметром)
)
...
function Do-Setup {
    ...
    # Фильтр по нашей ИБ: processName сопоставляется с BasePath
    $logcfgContent = @"
...
function Show-ManualSetupInstructions {
    ...
    Write-Host "    `$script = Get-ChildItem `$PWD -Recurse -Filter 'monitor-errors.ps1' | Select -First 1" -ForegroundColor DarkGray
...
function Parse-TJEvent {
    ...
    if ($processName -and $BasePath -and $processName -notmatch [regex]::Escape(($BasePath -replace '\\', '.'))) {
        return $null
    }
