...
4. Запустить: python _delete_object.py
5. Удалить временный скрипт после завершения

ПРИМЕР:
    # Удаление обработки ТестРеквизитов:
    files = [
        pathlib.Path(r"<PROJ_ROOT>\<Конфигурация>\DataProcessors\ТестРеквизитов.xml"),
    ]
    folders = [
        pathlib.Path(r"<PROJ_ROOT>\<Конфигурация>\DataProcessors\ТестРеквизитов"),
    ]
"""
...
BASE_PATH = pathlib.Path(__file__).resolve().parent  # корень проекта
