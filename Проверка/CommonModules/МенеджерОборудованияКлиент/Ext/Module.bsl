...
Функция ВерсияБиблиотеки() Экспорт
	...
	Возврат ПодключаемоеОборудование().ВерсияБиблиотеки;
...
Функция ПодключаемоеОборудование() Экспорт
	...
	Если ПодключаемоеОборудование = Неопределено Тогда
		...
		ПодключаемоеОборудование.Вставить("ВерсияБиблиотеки", "");
		...
		ПодключаемоеОборудование.Вставить("ПараметрыРабочегоМеста", МенеджерОборудованияКлиентСервер.НовыеПараметрыРабочегоМеста());
		ПодключаемоеОборудование.Вставить("ЕстьПодключаемоеОборудование", Ложь);
		ПодключаемоеОборудование.Вставить("MACАдреса", Новый Массив);
		...
		ПодключаемоеОборудование.Вставить("СистемаЛогирования", Неопределено);
		ПодключаемоеОборудование.Вставить("ПоправкаКВремениСеанса" , 0);      
		ПодключаемоеОборудование.Вставить("ПодключенныеКомпоненты", Новый Соответствие());
		ПодключаемоеОборудование.Вставить("ЛокальнаяОбработкаОшибок", Новый Массив());                
		ПодключаемоеОборудование.Вставить("КлассификаторыБиблиотеки", Новый Структура);
		ПодключаемоеОборудование.Вставить("СтатусыКассовыхСмен", Новый Соответствие);
		ПодключаемоеОборудование.Вставить("УведомленияКлиентаДоступно", Ложь);
		ЗаполнитьЛокальнуОбработкуОшибок(ПодключаемоеОборудование.ЛокальнаяОбработкаОшибок);
...
// Формирует массив локальных обраток ошибок.
// 
// Параметры:
//  ЛокальнаяОбработкаОшибок - Массив из Структура
//
Процедура ЗаполнитьЛокальнуОбработкуОшибок(ЛокальнаяОбработкаОшибок) Экспорт
	
	ТекстСообщения = НСтр("ru = 'Пожалуйста, проверьте данные для формирования данного способа расчета.'");
	
	ЛокальнаяОшибка = Новый Структура();     
	ЛокальнаяОшибка.Вставить("ТекстОшибки", НСтр("ru = 'При расчете в сети'")); 
	ЛокальнаяОшибка.Вставить("ТекстСообщения", ТекстСообщения);      
	ЛокальнаяОбработкаОшибок.Добавить(ЛокальнаяОшибка);
	
	ТекстСообщения = НСтр("ru = 'Пожалуйста, проверьте наличие бумаги в печатающем устройстве и повторно выполните операцию.'");
	
	ЛокальнаяОшибка = Новый Структура();     
	ЛокальнаяОшибка.Вставить("ТекстОшибки", НСтр("ru = 'Нет бумаги'")); 
	ЛокальнаяОшибка.Вставить("ТекстСообщения", ТекстСообщения);      
	ЛокальнаяОбработкаОшибок.Добавить(ЛокальнаяОшибка);
	
	ЛокальнаяОшибка = Новый Структура();     
	ЛокальнаяОшибка.Вставить("ТекстОшибки", НСтр("ru = 'Открыта крышка'")); 
	ЛокальнаяОшибка.Вставить("ТекстСообщения", ТекстСообщения);      
	ЛокальнаяОбработкаОшибок.Добавить(ЛокальнаяОшибка);           
	
	ЛокальнаяОшибка = Новый Структура();     
	ЛокальнаяОшибка.Вставить("ТекстОшибки", НСтр("ru = 'Ошибка отрезчика'")); 
	ЛокальнаяОшибка.Вставить("ТекстСообщения", ТекстСообщения);      
	ЛокальнаяОбработкаОшибок.Добавить(ЛокальнаяОшибка);           
	
	ЛокальнаяОшибка = Новый Структура();     
	ЛокальнаяОшибка.Вставить("ТекстОшибки", НСтр("ru = 'Механическая ошибка печатающего устройства'")); 
	ЛокальнаяОшибка.Вставить("ТекстСообщения", ТекстСообщения);      
	ЛокальнаяОбработкаОшибок.Добавить(ЛокальнаяОшибка);
	
	ТекстСообщения = НСтр("ru = 'Ресурс хранения ФД исчерпан. Данные не передавались в ОФД последние 30 дней.'");
	
	ЛокальнаяОшибка = Новый Структура();     
	ЛокальнаяОшибка.Вставить("ТекстОшибки", НСтр("ru = 'Ресурс хранения фд исчерпан'")); 
	ЛокальнаяОшибка.Вставить("ТекстСообщения", ТекстСообщения);     
	ЛокальнаяОбработкаОшибок.Добавить(ЛокальнаяОшибка);             

	ТекстСообщения = НСтр("ru = 'Возможно, истёк срок действия фискального накопителя. Пожалуйста, обратитесь в организацию обслуживающую ККТ.'");
	
	ЛокальнаяОшибка = Новый Структура();     
	ЛокальнаяОшибка.Вставить("ТекстОшибки", НСтр("ru = 'Исчерпал ресурс КС ФН'")); 
	ЛокальнаяОшибка.Вставить("ТекстСообщения", ТекстСообщения);     
	ЛокальнаяОбработкаОшибок.Добавить(ЛокальнаяОшибка);            
	
	ТекстСообщения = НСтр("ru = 'Необходимо закрыть смену на ККТ для дальнейшей работы.'");
	
	ЛокальнаяОшибка = Новый Структура();     
	ЛокальнаяОшибка.Вставить("ТекстОшибки", НСтр("ru = 'Смена превысила 24 часа'")); 
	ЛокальнаяОшибка.Вставить("ТекстСообщения", ТекстСообщения);     
	ЛокальнаяОбработкаОшибок.Добавить(ЛокальнаяОшибка);             
	
	ТекстСообщения = НСтр("ru = 'Необходимо открыть смену на ККТ для дальнейшей работы.'");     
	
	ЛокальнаяОшибка = Новый Структура();     
	ЛокальнаяОшибка.Вставить("ТекстОшибки", НСтр("ru = 'Кассовая смена не открыта или истекла'")); 
	ЛокальнаяОшибка.Вставить("ТекстСообщения", ТекстСообщения);     
	ЛокальнаяОбработкаОшибок.Добавить(ЛокальнаяОшибка);             

	ТекстСообщения = НСтр("ru = 'Контрольная марка не была проверена ранее.'");
	
	ЛокальнаяОшибка = Новый Структура();     
	ЛокальнаяОшибка.Вставить("ТекстОшибки", НСтр("ru = 'Контрольная марка не проверена'")); 
	ЛокальнаяОшибка.Вставить("ТекстСообщения", ТекстСообщения);     
	ЛокальнаяОбработкаОшибок.Добавить(ЛокальнаяОшибка);
	
	ТекстСообщения = НСтр("ru = 'Кассовая смена на ККТ была открыта ранее.'");
	
	ЛокальнаяОшибка = Новый Структура();     
	ЛокальнаяОшибка.Вставить("ТекстОшибки", НСтр("ru = 'Смена открыта'")); 
	ЛокальнаяОшибка.Вставить("ТекстСообщения", ТекстСообщения);        
	ЛокальнаяОбработкаОшибок.Добавить(ЛокальнаяОшибка);
	
	ЛокальнаяОшибка = Новый Структура();     
	ЛокальнаяОшибка.Вставить("ТекстОшибки", НСтр("ru = 'Кассовая смена уже открыта'")); 
	ЛокальнаяОшибка.Вставить("ТекстСообщения", ТекстСообщения);      
	ЛокальнаяОбработкаОшибок.Добавить(ЛокальнаяОшибка);   
	
	ТекстСообщения = НСтр("ru = 'Кассовая смена на ККТ была закрыта ранее.'");
	
	ЛокальнаяОшибка = Новый Структура();     
	ЛокальнаяОшибка.Вставить("ТекстОшибки", НСтр("ru = 'Смена закрыта'")); 
	ЛокальнаяОшибка.Вставить("ТекстСообщения", ТекстСообщения);       
	ЛокальнаяОбработкаОшибок.Добавить(ЛокальнаяОшибка);
	
	ЛокальнаяОшибка = Новый Структура();     
	ЛокальнаяОшибка.Вставить("ТекстОшибки", НСтр("ru = 'Кассовая смена уже закрыта'")); 
	ЛокальнаяОшибка.Вставить("ТекстСообщения", ТекстСообщения);        
	ЛокальнаяОбработкаОшибок.Добавить(ЛокальнаяОшибка);          
	
	ЛокальнаяОшибка = Новый Структура();     
	ЛокальнаяОшибка.Вставить("ТекстОшибки", НСтр("ru = '73h'")); 
	ЛокальнаяОшибка.Вставить("ТекстСообщения", ТекстСообщения);        
	ЛокальнаяОбработкаОшибок.Добавить(ЛокальнаяОшибка);          
	
	ТекстСообщения = НСтр("ru = 'Превышен максимальный допустимый размер чека. Необходимо уменьшить количество фискальных позиций в чеке..'");
	
	ЛокальнаяОшибка = Новый Структура();     
	ЛокальнаяОшибка.Вставить("ТекстОшибки", НСтр("ru = 'Размер чека'")); 
	ЛокальнаяОшибка.Вставить("ТекстСообщения", ТекстСообщения);      
	ЛокальнаяОбработкаОшибок.Добавить(ЛокальнаяОшибка);  
	
	ТекстСообщения = НСтр("ru = 'Пожалуйста, проверьте подключение оборудования и повторно выполните операцию.'");
	
	ЛокальнаяОшибка = Новый Структура();     
	ЛокальнаяОшибка.Вставить("ТекстОшибки", НСтр("ru = 'Нет связи'")); 
	ЛокальнаяОшибка.Вставить("ТекстСообщения", ТекстСообщения);      
	ЛокальнаяОбработкаОшибок.Добавить(ЛокальнаяОшибка);   
	
	ЛокальнаяОшибка = Новый Структура();     
	ЛокальнаяОшибка.Вставить("ТекстОшибки", НСтр("ru = 'FFFFFFFFh, Нет связи'")); 
	ЛокальнаяОшибка.Вставить("ТекстСообщения", ТекстСообщения);      
	ЛокальнаяОбработкаОшибок.Добавить(ЛокальнаяОшибка);   
	
	ЛокальнаяОшибка = Новый Структура();     
	ЛокальнаяОшибка.Вставить("ТекстОшибки", НСтр("ru = 'Порт недоступен'")); 
	ЛокальнаяОшибка.Вставить("ТекстСообщения", ТекстСообщения);      
	ЛокальнаяОбработкаОшибок.Добавить(ЛокальнаяОшибка);   
	
	ЛокальнаяОшибка = Новый Структура();     
	ЛокальнаяОшибка.Вставить("ТекстОшибки", НСтр("ru = 'FFFFFFF8h, Connect timed out'")); 
	ЛокальнаяОшибка.Вставить("ТекстСообщения", ТекстСообщения);      
	ЛокальнаяОбработкаОшибок.Добавить(ЛокальнаяОшибка);   
	
	ЛокальнаяОшибка = Новый Структура();     
	ЛокальнаяОшибка.Вставить("ТекстОшибки", НСтр("ru = 'FFFFFFFEh, СOM порт недоступен'")); 
	ЛокальнаяОшибка.Вставить("ТекстСообщения", ТекстСообщения);      
	ЛокальнаяОбработкаОшибок.Добавить(ЛокальнаяОшибка);   
	
	ЛокальнаяОшибка = Новый Структура();     
	ЛокальнаяОшибка.Вставить("ТекстОшибки", НСтр("ru = 'Нет связи с устройством'")); 
	ЛокальнаяОшибка.Вставить("ТекстСообщения", ТекстСообщения);      
	ЛокальнаяОбработкаОшибок.Добавить(ЛокальнаяОшибка);   
	
	ТекстСообщения = НСтр("ru = 'Обновите основную поставку драйвера, выполните обновление прошивки ККТ. Если проблема повторяется, требуется обращение в техническую поддержку разработчика драйвера.'");
	
	ЛокальнаяОшибка = Новый Структура();
	ЛокальнаяОшибка.Вставить("ТекстОшибки", НСтр("ru = 'tax'"));
	ЛокальнаяОшибка.Вставить("ТекстСообщения", ТекстСообщения);
	ЛокальнаяОбработкаОшибок.Добавить(ЛокальнаяОшибка);
...
Функция ОбновитьРабочееМестоКлиента() Экспорт
	...
	Если Не ЗначениеЗаполнено(РабочееМесто) Тогда
		...
		ПодключаемоеОборудование = ПодключаемоеОборудование();
		MACАдреса = ПодключаемоеОборудование.MACАдреса;
		...
		Если ОбщегоНазначенияБПОКлиент.ЭтоВебКлиент() И ЕстьПодключаемоеОборудование Тогда  
			Если MACАдреса.Количество() = 0 Тогда
				Оповещение = Новый ОписаниеОповещения("НачатьПолучениеMACАдресаЗавершение", ЭтотОбъект);
				УстанавливатьРасширениеБраузера = Истина;
				НачатьПолучениеMACАдреса(Оповещение, УстанавливатьРасширениеБраузера);
			Иначе
				НачатьПолучениеMACАдресаЗавершение(MACАдреса, Неопределено);
			КонецЕсли;
		Иначе
			РеквизитыРабочегоМеста = АктуальныеРеквизитыРабочегоМеста();
			РабочееМесто = МенеджерОборудованияВызовСервера.СоздатьРабочееМестоКлиента(РеквизитыРабочегоМеста);
...
Процедура УстановитьРабочееМесто(РабочееМесто) Экспорт
	...
	ПодключаемоеОборудование = ПодключаемоеОборудование();
	ПодключаемоеОборудование.ПараметрыРабочегоМеста.РабочееМесто = РабочееМесто;
...
Функция РабочееМестоКлиента(ЗапрашиватьНаСервере = Истина) Экспорт
	...
	ПодключаемоеОборудование = ПодключаемоеОборудование();
	РабочееМесто = ПодключаемоеОборудование.ПараметрыРабочегоМеста.РабочееМесто;
...
// Возвращает структру возможных типов оборудования
// 
// Возвращаемое значение:
//  Структура
Функция ПараметрыТипыОборудования() Экспорт
	
	ТипыОборудования = МенеджерОборудованияКлиентПовтИсп.ПараметрыТипыОборудования();
	Возврат ОбщегоНазначенияБПОКлиент.СкопироватьРекурсивно(ТипыОборудования);
	
КонецФункции

// Возвращает типы оборудования в виде массива перечислений
// 
// Параметры:
//   ТипыПО - Массив из Строка, Строка, Структура - тип оборудования.
// 
// Возвращаемое значение:
//   Массив из ОпределяемыйТип.ТипыПодключаемогоОборудования
//           
Функция МассивТиповОборудования(ТипыПО) Экспорт
	
	МассивТипов = Новый Массив();
	СоответствиеТиповОборудования = СоответствиеТиповОборудования();
	Если ТипЗнч(ТипыПО) = Тип("Строка") Тогда
		
		ТипыВМассиве = СтрРазделить(ТипыПО, ",", Ложь);
		Для Каждого ТипОборудования Из ТипыВМассиве Цикл
			МассивТипов.Добавить(СоответствиеТиповОборудования.Получить(СокрЛП(ТипОборудования)));
		КонецЦикла;
		
	ИначеЕсли ТипЗнч(ТипыПО) = Тип("Массив") Тогда
		
		ДоступныеТипыОборудования = ДоступныеТипыОборудования();
	...
	ИначеЕсли ТипЗнч(ТипыПО) = Тип("Структура") Тогда
		...
		Для Каждого КлючИЗначение Из ТипыПО Цикл
			...
			Если КлючИЗначение.Значение Тогда
				МассивТипов.Добавить(СоответствиеТиповОборудования.Получить(КлючИЗначение.Ключ));
			КонецЕсли;
...
Функция ПолучитьПодключенныеУстройства(ТипыПодключаемогоОборудования) Экспорт
	...
	Если ТипЗнч(ТипыПодключаемогоОборудования) = Тип("Структура") Тогда  
		...
		КонецЦикла;
	Иначе
		ТипыОборудования = МассивТиповОборудования(ТипыПодключаемогоОборудования);
		Для Каждого Подключение Из СписокПодключений Цикл
...
//
// Возвращаемое значение:
//   Структура - описание драйвера оборудования
//
Асинх Функция ПолучениеОписанияДрайвераАсинх(ИдентификаторДрайвера = Неопределено, ДанныеДрайвера = Неопределено) Экспорт
	...
	ОбработчикДрайвера = НовоеПодключение.ОбработчикДрайвера;
	Результат = Ждать ОбработчикДрайвера.ОписаниеДрайвераАсинх(НовоеПодключение);
...
//  СетевоеОборудование - Булево - использовать сетевое оборудование при подключении.
//
Процедура НачатьПодключениеОборудования(ОповещениеПриПодключении, ИдентификаторКлиента, ТипыПО = Неопределено, ИдентификаторУстройства = Неопределено, СетевоеОборудование = Ложь) Экспорт
...
Процедура НачатьОтключениеОборудования(ОповещениеПриОтключении, ИдентификаторКлиента, ТипыПО = Неопределено, ИдентификаторУстройства = Неопределено) Экспорт
	...
	Для Индекс = 0 По ПодключенныеУстройства.ВГраница() Цикл
		ОтключаемоеУстройство = ПодключенныеУстройства[Индекс];
		КлиентПодключения = ОтключаемоеУстройство.Клиенты.Найти(ИдентификаторКлиента);
		Если КлиентПодключения <> Неопределено Тогда  
			Если ОтключаемоеУстройство.КоличествоПодключенных = 1 Тогда
				Контекст = Новый Структура();
				...
				Контекст.Вставить("ЭтоОтключениеПоследнегоДрайвера", (ПодключенныеУстройства.ВГраница() = Индекс));
...
Процедура НачатьОтключениеОборудованияЗавершение(РезультатВыполнения, Контекст) Экспорт
	...
	Если Контекст.ЭтоОтключениеПоследнегоДрайвера И ОбщегоНазначенияБПОКлиент.ЕстьДанныеДляЗаписиОперацийОборудования() Тогда
		ОбменДаннымиССервером();
	КонецЕсли;
...
Процедура НачатьОтключениеВсегоОборудования(ОповещениеПриОтключении = Неопределено) Экспорт
	...
	Для Индекс = 0 По ПараметрыПодключенияПО.ВГраница() Цикл
		ПодключенноеУстройство = ПараметрыПодключенияПО[Индекс];
		Контекст = Новый Структура();
		Контекст.Вставить("ЭтоОтключениеПоследнегоДрайвера", (ПараметрыПодключенияПО.ВГраница() = Индекс));
		Оповещение = Новый ОписаниеОповещения("НачатьОтключениеВсегоОборудованияЗавершение", ЭтотОбъект, Контекст);
...
Процедура НачатьОтключениеВсегоОборудованияЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если НЕ РезультатВыполнения.Результат Тогда
		...
		ОбщегоНазначенияБПОКлиент.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
	Если Параметры.ЭтоОтключениеПоследнегоДрайвера Тогда
		ОбменДаннымиССервером();
	КонецЕсли;
	...
	Иначе
		Если СуществуютРеквизитыНаФорме(Форма, "ПоддерживаемыеТипыПодключаемогоОборудования") Тогда
			НачатьОтключениеОборудованияПоТипу(
				...
				Форма.УникальныйИдентификатор,
				МассивТиповОборудования(Форма.ПоддерживаемыеТипыПодключаемогоОборудования));
		КонецЕсли;
	КонецЕсли;
...
Асинх Функция ОтключитьОборудованиеПриЗакрытииФормыАсинх(Форма) Экспорт
	...
	ПодключаемоеОборудование = ПодключаемоеОборудованиеНаФорме(Форма);
	Если ПодключаемоеОборудование <> Неопределено Тогда
		Результат = Ждать ОтключениеОборудованияАсинх(Форма.УникальныйИдентификатор, ПодключаемоеОборудование.ТипыПО);
	КонецЕсли;
...
Процедура ВыполнениеКомандыОбработчика(ПодключенноеУстройство, ПараметрыВыполнениеКоманды) Экспорт
	...
	Если ПараметрыВыполнениеКоманды.ПодготовитьДанные Или ПараметрыВыполнениеКоманды.ПодготовитьДанныеКлиент Тогда
		...
		Если ПараметрыВыполнениеКоманды.ПодготовитьДанные Тогда
			...
			КонецЕсли; 
		КонецЕсли;
		Если ПараметрыВыполнениеКоманды.ПодготовитьДанныеКлиент Тогда
			ПараметрыПодключенияСервер = МенеджерОборудованияКлиентСервер.ПараметрыПодключения(ПодключенноеУстройство);
			ПроцессорДанных = ПараметрыВыполнениеКоманды.ПроцессорДанных + "Клиент";
			// Подготовка данных для проведения операции на сервере.
			ДанныеОперации = ПодготовитьДанныеОперации(ПараметрыПодключенияСервер, ПроцессорДанных, Команда, ПараметрыВыполнениеКоманды.ПараметрыОперации, ДанныеОперации); 
...
Процедура НачатьВыполнениеКоманды_ЗавершениеПечати(РезультатВыполнения, ПараметрыВыполнениеКоманды)  Экспорт
	...
	ДанныеЕГАИС.Вставить("Выполнено", РезультатВыполнения.ПродолжитьПечать);
	ДанныеЕГАИС.Вставить("ОписаниеОшибки", "");
	ДанныеЕГАИС.Вставить("АдресЧека", "");
	ДанныеЕГАИС.Вставить("ПодписьЧека", "");
	ДанныеЕГАИС.Вставить("ВходящееСообщение");
	ДанныеЕГАИС.Вставить("ИсходящееСообщение");
	ДанныеЕГАИС.Вставить("СообщениеПередано", Ложь);
	ЗаполнитьЗначенияСвойств(ДанныеЕГАИС, РезультатВыполнения);
	ДанныеОперации = ПараметрыВыполнениеКоманды.ДанныеОперации;
	ДанныеОперации.Вставить("ЗаписьОперацииЕГАИС", ДанныеЕГАИС);
...
Процедура НачатьВыполнениеКоманды_ОтменаОперацииИЗавершение(РезультатВыполнения, ПараметрыВыполнениеКоманды) Экспорт
	...
	Если ОбщегоНазначенияБПОКлиент.ИспользуетсяПлатежныеСистемы() Тогда
		...
		ЛогированиеОперацийБПОСлужебныйВызовСервера.ЗаписатьПлатежнуюОперацию(ПредопределенноеЗначение("Перечисление.ТипыПлатежныхОпераций.АварийнаяОтмена"), ПараметрыОперации, РезультатВыполнения);
	КонецЕсли;
...
Процедура ОтключитьОборудованиеПослеВыполнениеКоманды_ОтключитьПечатающееУстройствоЗавершение(РезультатОтключения, КонтекстВыполнения) Экспорт
	...
	Если ПараметрыВыполнениеКоманды.Свойство("ОставитьПодключенным") И ПараметрыВыполнениеКоманды.ОставитьПодключенным Тогда
		...
		ОставитьПодключенным = ПараметрыВыполнениеКоманды.ОставитьПодключенным;
	КонецЕсли;
	
	Если ОтключениеОборудования Или (СетевоеОборудование И Не ОставитьПодключенным) Тогда
...
//  Структура
//
Функция ПодготовитьДанныеОперации(ПараметрыПодключения, ПроцессорДанных, Команда, ПараметрыОперации, ДанныеОперации) Экспорт 
	
	МодульПроцессорДанных = ОбщегоНазначенияБПОКлиент.ОбщийМодуль(ПроцессорДанных);
	Возврат МодульПроцессорДанных.ПодготовитьДанныеОперации(ПараметрыПодключения, Команда, ПараметрыОперации, ДанныеОперации);
...
// Открытие формы списка операция с подключаемым оборудованием
//
// Параметры:
//  ПодключаемоеОборудование - СправочникСсылка.ПодключаемоеОборудование
//
Процедура ОткрытьОперацииСОборудованием(ПодключаемоеОборудование) Экспорт
	
	// ++ НеМобильноеПриложение
	Если ОбщегоНазначенияБПОКлиент.ИспользуетсяПодсистемаЛогирования() Тогда
		ПараметрыФормы = Новый Структура();
		ПараметрыФормы.Вставить("Отбор", Новый Структура("Оборудование", ПодключаемоеОборудование));
...
// Выполняет асинхронную загрузку архива логов драйвера оборудования.
//
// Параметры:
//   ПодключаемоеОборудование - СправочникСсылка.ПодключаемоеОборудование - 
//      оборудование, для которого загружаются логи драйвера
//   ДрайверОборудования - СправочникСсылка.ДрайверыОборудования - 
//   	ссылка на драйвер оборудования
//
// Возвращаемое значение:
//   Структура - Результат загрузки архива логов, содержащий информацию об успехе операции и возможных ошибках:
//      * Успешно - Булево - Признак успешности загрузки архива логов
//      * ТекстОшибки - Строка - Текст ошибки, если операция не была успешно завершена
//      * ДанныеДрайвера - Структура, Неопределено - Данные загруженного драйвера
//
Асинх Функция ЗагрузитьАрхивЛоговДрайвераАсинх(ПодключаемоеОборудование, ДрайверОборудования, УстановитьРасширениеРаботыСфайлами = Ложь) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("Успешно", Ложь);
	Результат.Вставить("ТекстОшибки", "");
	Результат.Вставить("ДанныеДрайвера", Неопределено);
	
	Если Не ЗначениеЗаполнено(ПодключаемоеОборудование) Тогда
		Результат.ТекстОшибки = НСтр("ru='Не указано подключаемое оборудование.'");
		Возврат Результат;
	КонецЕсли;

	Если Не ЗначениеЗаполнено(ДрайверОборудования) Тогда
		Результат.ТекстОшибки = НСтр("ru='Не указан драйвер оборудования.'");
		Возврат Результат;
	КонецЕсли;
	
	Если ОбщегоНазначенияБПОКлиент.ЭтоВебКлиент() Тогда
		
		Если Не Ждать ПодключитьРасширениеРаботыСФайламиАсинх() Тогда
			
			ТекстОшибки = НСтр("ru='Ошибка подключения расширения работы с файлами. Загрузка недоступна.'");
			Если Не УстановитьРасширениеРаботыСфайлами Тогда
	...
	РезультатОписания = Ждать ПолучениеОписанияДрайвераАсинх(ДрайверОборудования);
	Если Не РезультатОписания.Результат Тогда
		Результат.ТекстОшибки = НСтр("ru = 'Не удалось получить описание драйвера. Рекомендуется переустановить драйвер.'");
		Возврат Результат;
	КонецЕсли;
	
	ОписаниеДрайвера = РезультатОписания.ОписаниеДрайвера;
	...
	Если Не ОписаниеДрайвера.ЛогДрайвераВключен Тогда
		Результат.ТекстОшибки = НСтр("ru = 'У драйвера отключено логирование операций. Рекомендуется включить логирование операций драйвера.'");
		Возврат Результат;
	КонецЕсли;
	
	Если ПустаяСтрока(ОписаниеДрайвера.ЛогДрайвераПутьКФайлу ) Тогда
		Результат.ТекстОшибки = НСтр("ru = 'У драйвера не задан путь к файлам логирования операций. Рекомендуется настроить логирование операций драйвера.'");
		Возврат Результат;
	КонецЕсли;
	...
	Если Не ФайлСуществует Тогда
		Результат.ТекстОшибки = НСтр("ru = 'Отсутсвуют файлы логирования. Рекомендуется проверить путь к логированию операций драйвера.'");
		Возврат Результат;
	КонецЕсли;
	...
	Если ЭтоКаталог Тогда
		...
		НайденныеФайлы  = СокрЛП(ОписаниеДрайвера.ЛогДрайвераПутьКФайлу);
		РазделительПути = ПолучитьРазделительПути();
		Если Прав(НайденныеФайлы, 1) <> РазделительПути Тогда
			НайденныеФайлы = НайденныеФайлы + РазделительПути;
		КонецЕсли;
		ПутьКФайлам = НайденныеФайлы;
		МаскаФайла = ПолучитьМаскуВсеФайлы();
		ОдинФайл = Ложь;
		
	Иначе
		// Лог файлы хранятся в одном файле
		ПутьКФайлам = Файл.ПолноеИмя;
		МаскаФайла = Неопределено;
		ОдинФайл = Истина;
	КонецЕсли;
	
	ДанныеАрхива = Новый Структура;
	ДанныеАрхива.Вставить("БазовыйКаталог", ПутьКФайлам);
	ДанныеАрхива.Вставить("ОдинФайл", ОдинФайл);
	
	Попытка
		Если ОбщегоНазначенияБПОКлиент.ЭтоВебКлиент() Тогда
			РезультатПередачи = Ждать ПередатьФайлыЛоговНаСервер(ПутьКФайлам, Маскафайла);
			Если РезультатПередачи.Успех Тогда
				Данные = РезультатПередачи.ОписанияПередаваемыхФайлов;
			Иначе
				Результат.ТекстОшибки = РезультатПередачи.ТекстОшибки;
				Возврат Результат;
			КонецЕсли;
		Иначе
			Данные = СоздатьАрхивЛоговНаКлиенте(ПутьКФайлам);
		КонецЕсли;
	
		МенеджерОборудованияВызовСервера.ЗаписатьАрхивЛоговДрайвера(
			ПодключаемоеОборудование, 
			Данные);
		Результат.Успешно = Истина;
		
	Исключение
		ШаблонОшибки =Нстр("ru = 'Не удалось сохранить архив логов. %1'"); 
		Результат.ТекстОшибки = СтрШаблон(ШаблонОшибки,
			ОбработкаОшибок.ПредставлениеОшибкиДляПользователя(ИнформацияОбОшибке()));
			
		ОбщегоНазначенияБПОКлиент.ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ru = 'Передача файлов логов драйверов.'"),
...
// Обрабатывает запрос полученный с сервевера, на архивацию логов драйвера оборудования.
//
// Параметры:
//   ДанныеЗапроса - Структура - данные запроса на архивацию логов:
//    * РабочееМесто - СправочникСсылка.РабочиеМеста
//    * ПодключаемоеОборудование - СправочникСсылка.ПодключаемоеОборудование
//    * ДрайверОборудования - СправочникСсылка.ДрайверыОборудования
//   ДополнительныеПараметры - Произвольный - дополнительные параметры обработки запроса
//
Асинх Процедура ОбработатьЗапросАрхиваЛоговДрайвера(ДанныеЗапроса, ДополнительныеПараметры) Экспорт
	
	Если ТипЗнч(ДанныеЗапроса) <> Тип("Структура") Тогда
		Возврат;
	КонецЕсли;
	
	Если ДанныеЗапроса.РабочееМесто <> РабочееМестоКлиента() Тогда
		Возврат;
	КонецЕсли;
	
	РезультатЗагрузки = Ждать ЗагрузитьАрхивЛоговДрайвераАсинх(
		ДанныеЗапроса.ПодключаемоеОборудование, 
		ДанныеЗапроса.ДрайверОборудования);
	
	МенеджерОборудованияВызовСервера.
		ОтправитьОтветЗаписиАрхиваЛоговДрайвера(ДанныеЗапроса, РезультатЗагрузки);
	
КонецПроцедуры

// Обрабатывает подтверждение на запрос архивации логов драйвера оборудования.
//
// Параметры:
//   ДанныеЗапроса - Структура - данные запроса на архивацию логов:
//    * ПодключаемоеОборудование - СправочникСсылка.ПодключаемоеОборудование - ссылка на экземпляр оборудования
//    * ДрайверОборудования - СправочникСсылка.ДрайверыОборудования - ссылка на драйвер оборудования
//   ДополнительныеПараметры - Произвольный - дополнительные параметры обработки запроса
//
Асинх Процедура ОбработатьОтветНаЗапросАрхиваЛоговДрайвера(ДанныеОтвета) Экспорт
	
	РабочееМесто = МенеджерОборудованияКлиент.РабочееМестоКлиента();
	Если ДанныеОтвета.РабочееМестоОтправитель <> РабочееМесто Тогда
		Возврат;
	КонецЕсли;
	
	Оповестить("БПО.ЗагруженАрхивЛоговДрайвера");
	
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("ПодключаемоеОборудование", ДанныеОтвета.ПодключаемоеОборудование);
	ДополнительныеПараметры.Вставить("УникальныйИдентификаторФормы", ДанныеОтвета.УникальныйИдентификаторФормы);
	ПриНажатииОповещенияПользователя = Новый ОписаниеОповещения("ОбработатьОтветНаЗапросАрхиваЛоговДрайвераПриНажатииОповещения", ЭтотОбъект, ДополнительныеПараметры);
	
	Если ДанныеОтвета = Неопределено Тогда
		Заголовок = НСтр("ru = 'Ошибка.'");
		Текст = НСтр("ru = 'Нет ответа от клиента'");
	...
	ИначеЕсли ДанныеОтвета.Успешно Тогда
		Заголовок = НСтр("ru = 'Успех.'");
		Текст = НСтр("ru = 'Архив логов успешно получен.'");
		ПоказатьОповещениеПользователя(
			Заголовок, 
			ПриНажатииОповещенияПользователя,
			Текст,
	...
	Иначе
		Заголовок = НСтр("ru = 'Ошибка.'");
		Текст = ДанныеОтвета.ТекстОшибки;
		ПоказатьОповещениеПользователя(
			Заголовок, 
			ПриНажатииОповещенияПользователя,
			Текст,
...
Процедура ОбработатьОтветНаЗапросАрхиваЛоговДрайвераПриНажатииОповещения(ДополнительныеПараметры) Экспорт
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Ключ", ДополнительныеПараметры.ПодключаемоеОборудование);
	ОткрытьФорму("Справочник.ПодключаемоеОборудование.Форма.ФормаЭлемента", ПараметрыФормы, , ДополнительныеПараметры.УникальныйИдентификаторФормы);
...
// Устарела: следует использовать МассивТиповОборудования.
// Формирования массива типов оборудования
// 
// Параметры:
//   Типы - Массив из Строка, Строка - тип оборудования.
// Возвращаемое значение:
//   Массив из ПеречислениеСсылка.ТипыПодключаемогоОборудования -  
//           
Функция ПолучитьТипыОборудования(Типы) Экспорт
	
	Возврат МассивТиповОборудования(Типы); 
...
Функция ОбъектДрайвераИзКэша(ИдентификаторОбъекта) Экспорт
	...
	ПодключаемоеОборудование = ПодключаемоеОборудование();
	Результат = ПодключаемоеОборудование.ДрайверыОборудования.Получить(ИдентификаторОбъекта);
	
	Возврат Результат;
	
КонецФункции

Процедура ОбменДаннымиССервером() Экспорт
	
	ДанныеДляСервераПоЗначению = Новый Структура;
	...
	Если ОбщегоНазначенияБПОКлиент.ИспользуетсяПодсистемаЛогирования() Тогда
		...
		ЗаписиЛогирования = МодульЛогированиеОперацийБПОКлиент.ДанныеДляСервера();
		Если ЗначениеЗаполнено(ЗаписиЛогирования) Тогда
			ДанныеДляСервераПоЗначению.Вставить("ЗаписиЛогирования", ЗаписиЛогирования);
		КонецЕсли;
	КонецЕсли;
	
	СтатусыКассовыхСмен = ПодключаемоеОборудование().СтатусыКассовыхСмен;
	Для Каждого СтатусСмены Из СтатусыКассовыхСмен Цикл
		СтатусыКассовыхСмен.Вставить(СтатусСмены.Ключ, Неопределено);
	КонецЦикла;
	ДанныеДляСервера.Вставить("СтатусыКассовыхСмен", СтатусыКассовыхСмен);
	
	Попытка
		МенеджерОборудованияВызовСервера.ОбменССервером(ДанныеДляСервера, ДанныеДляСервераПоЗначению);
	Исключение
		ОбщегоНазначенияБПОКлиент.ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ru = 'Запись операций оборудования'"),
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
	
	ПодключаемоеОборудование().СтатусыКассовыхСмен = ДанныеДляСервера.СтатусыКассовыхСмен;
	
КонецПроцедуры


// Добавляет объект драйвера в локальный кеша клиентского приложения
// 
// Параметры:
//   ИдентификаторОбъекта - Строка -
//   ОбъектВнешнейКомпоненты - 
// 
Процедура ДобавитьДрайверВКэш(ИдентификаторОбъекта, ОбъектВнешнейКомпоненты) Экспорт
	
	ПодключаемоеОборудование = МенеджерОборудованияКлиент.ПодключаемоеОборудование();
	ПодключаемоеОборудование.ДрайверыОборудования.Вставить(ИдентификаторОбъекта, ОбъектВнешнейКомпоненты);
	
КонецПроцедуры

Функция УведомленияКлиентаДоступно() Экспорт

	Возврат ПодключаемоеОборудование().УведомленияКлиентаДоступно;
...
Процедура НачатьПолучениеMACАдресаЗавершение(MACАдреса, Контекст) Экспорт
	
	ПараметрыРабочегоМеста = ПодключаемоеОборудование().ПараметрыРабочегоМеста;
	СтароеРабочееМесто = ПараметрыРабочегоМеста.РабочееМесто;
	
	СоздаватьРабочееМесто = Истина;
	ОпределитьРабочееМесто(СоздаватьРабочееМесто);
	
	НовоеРабочееместо = ПараметрыРабочегоМеста.РабочееМесто;
	Если СтароеРабочееМесто <> НовоеРабочееместо Тогда
		Оповестить("ИзмененоРабочееМестоТекущегоСеанса", НовоеРабочееместо);
	КонецЕсли;
...
Процедура ОпределитьРабочееМесто(СоздаватьРабочееМесто = Ложь) Экспорт
	
	ПодключаемоеОборудование = МенеджерОборудованияКлиент.ПодключаемоеОборудование();
	ПараметрыРабочегоМеста = ПодключаемоеОборудование.ПараметрыРабочегоМеста;
	ЗаписанныеРеквизиты = ПараметрыРабочегоМеста.ЗаписанныеРеквизитыПоиска;
	АктуальныеРеквизиты = ПараметрыРабочегоМеста.АктуальныеРеквизитыПоиска;
	ИзменилисьЗаписанныеРеквизитыРабочегоМеста = Ложь;
	ИзменилисьАктуальныеРеквизитыРабочегоМеста = Ложь;
	
	// ++ НеМобильноеПриложение
	Если Не ОбщегоНазначенияБПОКлиент.ЭтоВебКлиент() Тогда
		Если ПустаяСтрока(АктуальныеРеквизиты.ИмяКомпьютера) Тогда
			АктуальныеРеквизиты.ИмяКомпьютера = ОбщегоНазначенияБПОКлиент.ИмяКомпьютераКлиента();
			ИзменилисьАктуальныеРеквизитыРабочегоМеста = Истина;
		КонецЕсли;
		Если ЗаписанныеРеквизиты.ИмяКомпьютера <> АктуальныеРеквизиты.ИмяКомпьютера Тогда
			ИзменилисьЗаписанныеРеквизитыРабочегоМеста = Истина;
		КонецЕсли;
	КонецЕсли;
	
	НайденныеMACАдреса = ПодключаемоеОборудование.MACАдреса;
	Если АктуальныеРеквизиты.MACАдреса.Количество() = 0 И НайденныеMACАдреса.Количество() > 0 Тогда
		АктуальныеРеквизиты.MACАдреса = НайденныеMACАдреса;
		ИзменилисьАктуальныеРеквизитыРабочегоМеста = Истина;
	КонецЕсли;
	
	Если АктуальныеРеквизиты.MACАдреса.Количество() > 0 Тогда
		MACАдресаОтличаются = Не ОбщегоНазначенияБПОКлиентСервер.МассивыСовпадают(
			ЗаписанныеРеквизиты.MACАдреса, АктуальныеРеквизиты.MACАдреса);
		Если MACАдресаОтличаются Тогда
			ИзменилисьЗаписанныеРеквизитыРабочегоМеста = Истина;
		КонецЕсли;
	КонецЕсли;
	// -- НеМобильноеПриложение
	
	РабочееМесто = ПараметрыРабочегоМеста.РабочееМесто;
	Если ЗначениеЗаполнено(РабочееМесто) Тогда
		Если ИзменилисьЗаписанныеРеквизитыРабочегоМеста Тогда
			МенеджерОборудованияВызовСервера.ОбновитьРеквизитыРабочегоМеста(РабочееМесто, АктуальныеРеквизиты);
			ПараметрыРабочегоМеста.ЗаписанныеРеквизитыПоиска = АктуальныеРеквизиты;
		КонецЕсли;
	Иначе
		Если ИзменилисьАктуальныеРеквизитыРабочегоМеста Или СоздаватьРабочееМесто Тогда
			РабочееМесто = МенеджерОборудованияВызовСервера.ОбновитьРабочееМестоКлиента(АктуальныеРеквизиты, СоздаватьРабочееМесто);
	...
	//  исходные данные:
	//   ИД, Имя компьютера, МАС-адрес пока не известен
	//   попытаться найти рабочее место по ИД, имени компьютера
	//   в веб клиенте не нужно искать по имени компьютера т.к. имя компьютера не получить
	
	
	// вызывается после получения МАС адресов
	//   ИД, Имя компьютера, МАС-адреса
	//   если заполнены рквизиты тогда найти рабочее место по ИД, имени компьютера, МАС адресам
	//   если отличаются актуальные реквизиты и реквизиты из БД
	//   перезаписать рабочее место
	
	// вызывается после получения МАС адресов и установке компоненты расширения
...
Процедура ОпределитьРабочееМестоRDP() Экспорт
	...
	РабочееМесто = ПараметрыРабочегоМеста.РабочееМесто;
	Если ЗначениеЗаполнено(РабочееМесто) Тогда
		ИдентификаторКлиента = ИдентификаторКлиентаДляРабочегоМеста();
		Если ЗаписанныеРеквизиты.ИдентификаторКлиента <> ИдентификаторКлиента Тогда
			Параметры = Новый Структура;
			Параметры.Вставить("ИдентификаторКлиента", ИдентификаторКлиента);
			Параметры.Вставить("ИмяКомпьютера", ЗаписанныеРеквизиты.ИмяКомпьютера);
			Параметры.Вставить("MACАдреса", ЗаписанныеРеквизиты.MACАдреса);
			ВопросСоздатьРабочееМестоАсинх(ПараметрыРабочегоМеста);
		КонецЕсли;
	Иначе
		СоздаватьРабочееМесто = Истина;
		РабочееМесто = МенеджерОборудованияВызовСервера.ОбновитьРабочееМестоКлиента(АктуальныеРеквизиты, СоздаватьРабочееМесто);
...
Функция КлассификаторыБиблиотеки() Экспорт
	...
	ПодключаемоеОборудование = МенеджерОборудованияКлиент.ПодключаемоеОборудование();
	Возврат ПодключаемоеОборудование.КлассификаторыБиблиотеки;
...
//  Булево.
//
Функция ОбработатьСобытиеОтНесертифицираванногоДрайвера(Подключение, ОписаниеСобытия, ОписаниеОшибки = "") Экспорт
	...
	Если ОбщегоНазначенияБПОКлиент.ИспользуетсяНесертифицированноеОборудование() Тогда
		...
		МодульОборудованиеНесертифицированноеКлиентПереопределяемый.ОбработатьСобытие(Подключение, Событие, Данные, СтандартнаяОбработка);
...
Процедура ВыполнитьУстановкуРасширенияСПомощьюБПО(ОбработкаОповещения)
	...
	Если ВыполнятьУстановку Тогда
		...
		Исключение
			ОбщегоНазначенияБПОКлиент.ЗаписатьОшибкуВЖурналРегистрации(
				НСтр("ru = 'Установка расширения БПО'"),
				НСтр("ru = 'Не удается открыть форму вопроса об установке расширения, возможно нет права просмотра для указанной формы.'"));
			Если ОбработкаОповещения <> Неопределено Тогда 
				ВыполнитьОбработкуОповещения(ОбработкаОповещения, Неопределено);
			КонецЕсли;     
		КонецПопытки;
...
Процедура НачатьПолучениеИнформацииОСетевыхАдаптерахЗавершение(Результат, ОбработкаОповещения) Экспорт
	...
	Для Каждого ИнформацияОбАдаптере Из Результат Цикл
		MACАдреса.Добавить(ИнформацияОбАдаптере.MACАдрес);
	КонецЦикла;
	ПодключаемоеОборудование().MACАдреса = MACАдреса;
	
	Если ОбработкаОповещения <> Неопределено Тогда 
		ВыполнитьОбработкуОповещения(ОбработкаОповещения, MACАдреса);
	КонецЕсли;
...
//
Функция УстройстваДляПодключения(ИдентификаторКлиента, ТипыПО = Неопределено, ИдентификаторУстройства = Неопределено, СетевоеОборудование = Ложь)
...
Процедура ЗаполнитьПоддерживаемыеТипыПодключаемогоОборудованияНаФорме(Форма, ПоддерживаемыеТипыПодключаемогоОборудования)
	...
	ИначеЕсли ТипЗнч(Форма.ПоддерживаемыеТипыПодключаемогоОборудования) = Тип("Массив") Тогда
		...
		ТипыОборудования = МассивТиповОборудования(ПоддерживаемыеТипыПодключаемогоОборудования);
...
Процедура ЗавершениеВыполнениеКоманды(РезультатОперации, ПараметрыВыполнениеКоманды) Экспорт
	...
	Если ПараметрыВыполнениеКоманды.Свойство("СообщенияВСлужбуТехническойПоддержки") 
		...
		Если Не РезультатОперации.Результат Тогда
			...
			РезультатАнализаОшибки = АнализТекстаОшибкиОборудования(ПараметрыВыполнениеКоманды.ИдентификаторУстройства, РезультатОперации.ОписаниеОшибки);
			РезультатОперации.ОписаниеОшибки = РезультатАнализаОшибки.ОписаниеОшибки;
			ЛокальнаяОбработкаОшибки = РезультатАнализаОшибки.ЛокальнаяОбработкаОшибки;
			Если Не ЛокальнаяОбработкаОшибки Тогда  
				...
				ОписаниеОшибки = РезультатАнализаОшибки.ОписаниеОшибки;
				КраткоеОписание = РезультатАнализаОшибки.КраткоеОписание;
				ДополнительныеДанные = Новый Структура();
				ДополнительныеДанные.Вставить("Операция",          ПараметрыВыполнениеКоманды.Команда);
				ДополнительныеДанные.Вставить("ОписаниеОшибки",    ОписаниеОшибки);
				...
				|
				|Создать сообщение об ошибке в службу поддержки?'"), ОписаниеОшибки);
				...
				Параметры.Вставить("ТекстОшибки",               КраткоеОписание + ". " + РезультатОперации.ОписаниеОшибки);
...
Функция АнализТекстаОшибкиОборудования(ИдентификаторУстройства, ОписаниеОшибки) 
	
	ПараметрыПодключения = ПолучитьПодключенноеУстройство(ИдентификаторУстройства);
	ПараметрыПодключенияСервер = Новый Структура;
	ПараметрыПодключенияСервер.Вставить("ИДУстройства");
	ПараметрыПодключенияСервер.Вставить("ВерсияДрайвера");
	Если ПараметрыПодключения <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(ПараметрыПодключенияСервер, ПараметрыПодключения);
	КонецЕсли;
	
	Возврат МенеджерОборудованияВызовСервера.АнализТекстаОшибкиОборудования(ИдентификаторУстройства, ПараметрыПодключенияСервер, ОписаниеОшибки);
...
Функция АктуальныеРеквизитыРабочегоМеста()
	
	РеквизитыРабочегоМеста = МенеджерОборудованияКлиентСервер.НовыеРеквизитыПоискаРабочегоМеста();
	
	РеквизитыРабочегоМеста.ИдентификаторКлиента = МенеджерОборудованияКлиентСервер.ИдентификаторКлиентаДляРабочегоМеста();
	РеквизитыРабочегоМеста.ИмяКомпьютера = ОбщегоНазначенияБПОКлиент.ИмяКомпьютераКлиента();
	РеквизитыРабочегоМеста.MACАдреса = ПодключаемоеОборудование().MACАдреса;
	
	Возврат РеквизитыРабочегоМеста;
	
КонецФункции

// ++ НеМобильноеПриложение 

Асинх Процедура ВопросСоздатьРабочееМестоАсинх(ПараметрыРабочегоМеста)
	...
	Если Ответ = "Создать" Тогда
		СоздаватьРабочееМесто = Истина;
		РабочееМесто = МенеджерОборудованияВызовСервера.СоздатьРабочееМестоКлиента(АктуальныеРеквизиты);
		ПараметрыРабочегоМеста.РабочееМесто = РабочееМесто;
	ИначеЕсли Ответ = "Обновить" Тогда
		МенеджерОборудованияВызовСервера.ОбновитьИдентификаторРабочегоМеста(
			ТекущееРМ, АктуальныеРеквизиты.ИдентификаторКлиента);
	КонецЕсли;
...
#Область АрхивЛоговДрайверов

// Создает архив логов на клиенте из файлов по указанному пути.
//
// Параметры:
//   ПутьКФайлам - Строка - Путь к файлам логов для архивации
//
// Возвращаемое значение:
//   ДвоичныеДанные, Неопределено - Архив логов, либо Неопределено в случае 
//    ошибки или при работе в веб-клиенте
//
Функция СоздатьАрхивЛоговНаКлиенте(ПутьКФайлам)
	
#Если Не ВебКлиент Тогда
	
	Попытка
		Архив = Новый ЗаписьZipФайла();
		Архив.Добавить(ПутьКФайлам, 
...
// Передает файлы логов на сервер.
//
// Параметры:
//   ПутьКФайлам - Строка - путь к каталогу с файлами логов для передачи
//   Маскафайла - Строка - маска для отбора файлов, по умолчанию Неопределено
//
// Возвращаемое значение:
//   Структура - результат передачи файлов:
//   
//
Асинх Функция ПередатьФайлыЛоговНаСервер(ПутьКФайлам, Маскафайла = Неопределено)
	
	Результат = Новый Структура;
	Результат.Вставить("Успех", Ложь);
	Результат.Вставить("ТекстОшибки", "");
	Результат.Вставить("ОписанияПередаваемыхФайлов", Новый Массив);
	
	Файлы = Ждать НайтиФайлыАсинх(ПутьКФайлам, МаскаФайла, Истина);
	ОписанияПередаваемыхФайлов = Новый Массив;
	Для Каждого Файл Из Файлы Цикл
		Если Не Ждать Файл.ЭтоКаталогАсинх() Тогда
			ОписанияПередаваемыхФайлов.Добавить(Новый ОписаниеПередаваемогоФайла (Файл.ПолноеИмя));
		КонецЕсли;
	КонецЦикла;
	
	Попытка
		ПереданныеФайлы = Ждать ПоместитьФайлыНаСерверАсинх(,,ОписанияПередаваемыхФайлов);
	Исключение
		ШаблонОшибки = НСтр("ru = 'Не удалось поместить файлы на сервер: %1'");
		ОбщегоНазначенияБПОКлиент.ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ru = 'Запись архива логов'"),
			СтрШаблон(ШаблонОшибки, ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
		Результат.ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Не удалось поместить файлы на сервер: %1'"),
			ОбработкаОшибок.ПредставлениеОшибкиДляПользователя(ИнформацияОбОшибке()));
		Возврат Результат;
	КонецПопытки;
	...
	Для Каждого Описание Из ПереданныеФайлы Цикл
		...
		ПереданныйФайл = Новый ОписаниеПередаваемогоФайла;
		ПереданныйФайл.Хранение = Описание.Адрес;
		ПереданныйФайл.Имя =  
			Сред(Описание.СсылкаНаФайл.Файл.ПолноеИмя, СтрДлина(ПутьКФайлам) + 1);
		Результат.ОписанияПередаваемыхФайлов.Добавить(ПереданныйФайл);
