...
Функция УведомленияКлиентаДоступно() Экспорт
	...
	Если ИспользуетсяБСП() Тогда
		...
		// Вызов БСП
		СерверныеОповещенияСлужебныйКлиентСерверМодуль = ОбщийМодуль("СерверныеОповещенияСлужебныйКлиентСервер");
		Возврат СерверныеОповещенияСлужебныйКлиентСерверМодуль.УведомленияКлиентаДоступны();
		// Конец Вызов БСП
		
	КонецЕсли;

	Возврат ОбщегоНазначенияБПОПовтИсп.УведомленияКлиентаДоступно();
...
Функция ОбщийМодуль(Имя) Экспорт
	...
	Если ИспользуетсяБСП() Тогда
		...
		// Вызов БСП
		ОбщегоНазначенияМодуль = ОбщийМодульБПО("ОбщегоНазначения");
...
Функция ИспользуютсяОблачныеККТ() Экспорт
	...
	И ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ОблачныеКассы");
...
//  Булево
//
Функция ПреобразоватьВБулево(Строка) Экспорт 
...
Функция СтрокуВJSON(Свойство, Значение, ДополнительныеПараметры, Отказ) Экспорт  
...
Функция ЗначенияРеквизитовОбъектаБПО(Ссылка, Знач Реквизиты)
	...
	|	&ТекстПолей
	|ИЗ
	|	&ПолноеИмя КАК ПсевдонимЗаданнойТаблицы
	|ГДЕ
	|	ПсевдонимЗаданнойТаблицы.Ссылка = &Ссылка";
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТекстПолей", ТекстПолей);
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ПолноеИмя", Ссылка.Метаданные().ПолноеИмя());
