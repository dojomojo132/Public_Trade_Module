...
Функция ПараметрыТипыОборудования() Экспорт
	...
	Возврат МенеджерОборудования.ПараметрыТипыОборудования();
	#Иначе
	Возврат МенеджерОборудованияКлиент.ПараметрыТипыОборудования();
	#КонецЕсли
...
//   *СообщениеПриИспользовании - Булево -  
//   *Предопределенный - Булево -
//   *ЛокальныйРежим - Булево -
//   *СетевойРежим - Булево -
//   *СерверныйРежим - Булево -
//   *БезопасноеПодключение - Булево -          
//   *РекомендуемоеИспользование - Булево - 
//
Функция ПараметрыСозданияНовогоДрайвера() Экспорт
	...
	ПараметрыСоздания.Вставить("БезопасноеПодключение", Ложь);      
	ПараметрыСоздания.Вставить("РекомендуемоеИспользование", Ложь);      
	ПараметрыСоздания.Вставить("ИмяМакетаДрайвера");
	ПараметрыСоздания.Вставить("СнятСПоддержки"  , Ложь);  
	ПараметрыСоздания.Вставить("СообщениеПриИспользовании" , Истина);  
...
// Параметры создания нового подключаемого оборудования.
// 
// Возвращаемое значение:
//  Структура - Параметры создания нового подключаемого оборудования:  
//   *Наименование - Строка -  
//   *ТипОборудования - ПеречислениеСсылка.ТипыПодключаемогоОборудования - 
//   *ТипПодключения - ПеречислениеСсылка.ТипыПодключенияОборудования -
//   *ДрайверОборудования - СправочникСсылка.ДрайверыОборудования - 
//   *Организация - ОпределяемыйТип.ОрганизацияБПО -   
//   *РабочееМесто - СправочникСсылка.РабочиеМеста -   
//   *СерийныйНомер - Строка -      
//   *СпособФорматноЛогическогоКонтроля - ПеречислениеСсылка.СпособыФорматноЛогическогоКонтроля -    
//   *ДопустимоеРасхождениеФорматноЛогическогоКонтроля - Число -   
//   *ИспользуетсяФН36 - Булево - 
//
Функция ПараметрыСозданияНовогоПодключаемогоОборудования() Экспорт
	
	ПараметрыСоздания = Новый Структура();           
	ПараметрыСоздания.Вставить("Наименование");
	ПараметрыСоздания.Вставить("УстройствоИспользуется", Истина);
	ПараметрыСоздания.Вставить("ТипОборудования"       , Неопределено);
	ПараметрыСоздания.Вставить("ТипПодключения"        , ПредопределенноеЗначение("Перечисление.ТипыПодключенияОборудования.ЛокальноеПодключение"));
	ПараметрыСоздания.Вставить("ДрайверОборудования"   , Неопределено);
	ПараметрыСоздания.Вставить("Организация"); 
	ПараметрыСоздания.Вставить("РабочееМесто", МенеджерОборудованияВызовСервера.РабочееМестоКлиента());
	ПараметрыСоздания.Вставить("СерийныйНомер");
	ПараметрыСоздания.Вставить("СпособФорматноЛогическогоКонтроля", ПредопределенноеЗначение("Перечисление.СпособыФорматноЛогическогоКонтроля.РазделятьСтроки"));;    
	ПараметрыСоздания.Вставить("ДопустимоеРасхождениеФорматноЛогическогоКонтроля", 0.01);      
	ПараметрыСоздания.Вставить("ИспользуетсяФН36", Ложь);
	Возврат ПараметрыСоздания;
...
Функция ПараметрыОписанияДрайвера() Экспорт
	...
	Результат.Вставить("РевизияИнтерфейсаПредставление");
	...
	Результат.Вставить("ИнформацияОбОкружении"    , "");            
	Результат.Вставить("АвтоНастройкаОборудования", Ложь);            
...
Функция ПараметрыПодключения(ДанныеДрайвера) Экспорт
	...
	Подключение.Вставить("СнятСПоддержки"           , ДанныеДрайвера.СнятСПоддержки);      
	Подключение.Вставить("СообщениеПриИспользовании", ДанныеДрайвера.СообщениеПриИспользовании); 
	Подключение.Вставить("БезопасноеПодключение"    , ДанныеДрайвера.БезопасноеПодключение);    
	...
	Подключение.Вставить("Несертифицированное"  , ДанныеДрайвера.Несертифицированное);
	
	Подключение.Вставить("РевизияИнтерфейса", ?(ДанныеДрайвера.Свойство("РевизияИнтерфейса"), ДанныеДрайвера.РевизияИнтерфейса, 0));  
	Подключение.Вставить("РевизияИнтерфейсаПредставление", ?(ДанныеДрайвера.Свойство("РевизияИнтерфейсаПредставление"), ДанныеДрайвера.РевизияИнтерфейсаПредставление, ""));  
	...
	Подключение.Вставить("ИдентификаторБанкаЭквайера"     , ?(ДанныеДрайвера.Свойство("ИдентификаторБанкаЭквайера"), ДанныеДрайвера.ИдентификаторБанкаЭквайера, ""));  
	...
	Подключение.Вставить("ПокупкаСЗачислениемНаКарту"     , ?(ДанныеДрайвера.Свойство("ПокупкаСЗачислениемНаКарту"), ДанныеДрайвера.ПокупкаСЗачислениемНаКарту, Ложь));  
	...
	Подключение.Вставить("ДисплейОтрабатываетНажатия"  , ?(ДанныеДрайвера.Свойство("ДисплейОтрабатываетНажатия"), ДанныеДрайвера.ДисплейОтрабатываетНажатия, Ложь));
...
Функция ПараметрыВыполнениеКоманды(Команда, ПроцессорДанных, ДополнительныеПараметры = Неопределено, ПодготовитьДанные = Истина, 
	...
	ПараметрыВыполнение.Вставить("ПодготовитьДанныеКлиент", Ложь);
	ПараметрыВыполнение.Вставить("ОбработатьДанныеКлиент" , Ложь);
...
Функция ОпределитьТипШтрихкода(Знач Штрихкод) Экспорт
	...
	Если ТипШтрихкодаЗнач = "CODE128"  Тогда // EAN128
		...
		Если КодСимвола(Штрихкод, 1) = 40 Или СтрНайти(Штрихкод, ОбщегоНазначенияБПОКлиентСервер.РазделительGS1()) > 0  Тогда
...
Функция НовыеРеквизитыПоискаРабочегоМеста() Экспорт
	
	Реквизиты = Новый Структура;
	Реквизиты.Вставить("ИдентификаторКлиента", "");
	Реквизиты.Вставить("ИмяКомпьютера", "");
	Реквизиты.Вставить("MACАдреса", Новый Массив);
	Возврат Реквизиты;
	
КонецФункции

Функция НовыеПараметрыРабочегоМеста() Экспорт
	
	Параметры = Новый Структура;
	Параметры.Вставить("РабочееМесто");
	Параметры.Вставить("ИспользуетсяRDP", Ложь);
	Параметры.Вставить("ЕстьПодключаемоеОборудование", Истина);
	
	Параметры.Вставить("АктуальныеРеквизитыПоиска", НовыеРеквизитыПоискаРабочегоМеста());
	Параметры.Вставить("ЗаписанныеРеквизитыПоиска", НовыеРеквизитыПоискаРабочегоМеста());
	
	Возврат Параметры;
	
КонецФункции

// Возвращает ключ запроса архива логов драйвера, для оповещения клиента.
//
// Возвращаемое значение:
//   Строка
//
Функция КлючЗапросаАрхиваЛоговДрайвера() Экспорт
	Возврат "БПО.ЗапросАрхиваЛоговДрайвера";
КонецФункции

// Возвращает ключ ответа на запрос архива логов драйвера, для оповещения клиента.
//
// Возвращаемое значение:
//   Строка
//
Функция КлючОтветаНаЗапросАрхиваЛоговДрайвера() Экспорт
	Возврат "БПО.ОтветНаЗапросАрхиваЛоговДрайвера";
КонецФункции
